...
if (( $# < 3 )); then
  echo 'Usage: utxo_snapshot.sh <generate-at-height> <snapshot-out-path> <fjarcode-cli-call ...>'
  echo
  ...
  echo 'Examples:'
  echo
  echo "  ./contrib/devtools/utxo_snapshot.sh 570000 utxo.dat ./src/fjarcode-cli -datadir=\$(pwd)/testdata"
  echo '  ./contrib/devtools/utxo_snapshot.sh 570000 - ./src/fjarcode-cli'
  exit 1
fi
...
FJARCODE_CLI_CALL="${*} -rpcclienttimeout=9999999"

# Check if the node is pruned and get the pruned block height
PRUNED=$( ${FJARCODE_CLI_CALL} getblockchaininfo | awk '/pruneheight/ {print $2}' | tr -d ',' )
...
function cleanup {
  ...
  ${FJARCODE_CLI_CALL} reconsiderblock "${PIVOT_BLOCKHASH}"

  if $NETWORK_DISABLED; then
    (>&2 echo "Restoring network activity")
    ${FJARCODE_CLI_CALL} setnetworkactive true
  fi
}
...
if [[ "$REPLY" =~ ^[Yy]*$ || -z "$REPLY" ]]; then
  ...
  ${FJARCODE_CLI_CALL} setnetworkactive false
else
  (>&2 echo "Network activity remains enabled")
fi

# Block we'll invalidate/reconsider to rewind/fast-forward the chain.
PIVOT_BLOCKHASH=$($FJARCODE_CLI_CALL getblockhash $(( GENERATE_AT_HEIGHT + 1 )) )
...
${FJARCODE_CLI_CALL} invalidateblock "${PIVOT_BLOCKHASH}"

if [[ "${OUTPUT_PATH}" = "-" ]]; then
  (>&2 echo "Generating txoutset info...")
  ${FJARCODE_CLI_CALL} gettxoutsetinfo | grep hash_serialized_3 | sed 's/^.*: "\(.\+\)\+",/\1/g'
else
  (>&2 echo "Generating UTXO snapshot...")
  ${FJARCODE_CLI_CALL} dumptxoutset "${OUTPUT_PATH}"
fi
