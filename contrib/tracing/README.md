...
The bpftrace examples contain a relative path to the `fjarcoded` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `fjarcoded` binary. The paths in the examples can be changed, for
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/fjarcoded
```
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/fjarcoded
```
...
…
Warning: incomplete message (only 32568 out of 53552 bytes)! inbound msg 'tx' from peer 32 (outbound-full-relay, XX.XXX.XXX.43:8338) with 53552 bytes: 020000000001fd3c01939c85ad6756ed9fc…
…
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py ./src/fjarcoded
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py ./src/fjarcoded
```
