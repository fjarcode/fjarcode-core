...
def main(fjarcoded_path):
    fjarcoded_with_usdts = USDT(path=str(fjarcoded_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    fjarcoded_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    fjarcoded_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    fjarcoded_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    fjarcoded_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[fjarcoded_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/fjarcoded")
        exit(1)
