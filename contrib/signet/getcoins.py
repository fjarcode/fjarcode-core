...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing fjarcode-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='fjarcode-cli', help='fjarcode-cli command to use')
...
parser.add_argument('fjarcode_cli_args', nargs='*', help='Arguments to pass on to fjarcode-cli (default: -signet)')

args = parser.parse_args()

if args.fjarcode_cli_args == []:
    args.fjarcode_cli_args = ['-signet']


def fjarcode_cli(rpc_command_and_params):
    argv = [args.cmd] + args.fjarcode_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = fjarcode_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = fjarcode_cli(['getnewaddress', 'faucet', 'bech32'])
