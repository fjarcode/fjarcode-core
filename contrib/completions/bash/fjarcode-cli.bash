# bash programmable completion for fjarcode-cli(1)
# Copyright (c) 2012-2022 The Bitcoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

# call $fjarcode-cli for RPC
_codefalcon_rpc() {
    ...
    $codefalcon_cli "${rpcargs[@]}" "$@"
}

_codefalcon_cli() {
    local cur prev words=() cword
    local codefalcon_cli

    # save and use original argument to invoke fjarcode-cli for -help, help and RPC
    # as fjarcode-cli might not be in $PATH
    codefalcon_cli="$1"
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($codefalcon_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_codefalcon_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
...
complete -F _codefalcon_cli fjarcode-cli
