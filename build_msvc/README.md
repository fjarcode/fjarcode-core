...
To build Bitcoin Core without Qt, unload or disable the `fjarcode-qt`, `libfjarcode_qt` and `test_fjarcode-qt` projects.
...
```cmd
msbuild build_msvc\fjarcode.sln -property:Configuration=Release -maxCpuCount -verbosity:minimal
```

Alternatively, open the `build_msvc/fjarcode.sln` file in Visual Studio.
...
To check if `fjarcoded` has `RandomizedBaseAddress` enabled or disabled run

```
.\dumpbin.exe /headers src/fjarcoded.exe
```
