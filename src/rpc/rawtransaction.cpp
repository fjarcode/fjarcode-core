...
// witness script types) are retained for parsing historical transactions and blocks.
// After FJAR fork, transactions have no witness data; hash equals txid, vsize equals size.
                     ...
                     TxVerbosity verbosity = TxVerbosity::SHOW_DETAILS)
{
    ...
    // Call into TxToUniv() in fjarcode-common to decode the transaction hex.
    //
    // Blockchain contextual information (confirmations and blocktime) is not
    // available to code in fjarcode-common, so we query them here and push the
...
static std::vector<RPCArg> CreateTxDoc()
{
    return {
        ...
        {"replaceable", RPCArg::Type::BOOL, RPCArg::Default{false}, "Marks this transaction as BIP125-replaceable.\n"
                "Note: RBF is not supported on FJAR after the fork. This parameter controls sequence number selection only.\n"
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    const bool can_wrap{[&] {
        ...
        for (CScript::const_iterator it{script.begin()}; it != script.end();) {
            ...
            if (op == OP_CHECKDATASIG || IsOpSuccess(op)) {
                return false;
            }
        }
        return true;
    ...
    if (can_wrap) {
        ...
        // After FJAR fork, SegWit transactions are not valid on-chain.
        if (can_wrap_P2WSH) {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    ChainstateManager& chainman = EnsureChainman(node);
    int nCurrentHeight = WITH_LOCK(cs_main, return chainman.ActiveChain().Height());

    UniValue result(UniValue::VOBJ);
    SignTransaction(mtx, &keystore, coins, request.params[3], result, nCurrentHeight);
    return result;
},
    };
}
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    CMutableTransaction mtx;
    bool complete = FinalizeAndExtractPSBT(psbtx, mtx, sighash_type);
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (complete) {
        ...
        CHECK_NONFATAL(FinalizeAndExtractPSBT(psbtx_copy, mtx, sighash_type));
