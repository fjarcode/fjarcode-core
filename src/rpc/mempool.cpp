        ...
        "\nRelated RPCs: createrawtransaction, signrawtransactionwithkey\n",
        {
            ...
            {"maxfeerate", RPCArg::Type::AMOUNT, RPCArg::Default{FormatMoney(DEFAULT_MAX_RAW_TX_FEE_RATE.GetFeePerK())},
             "Reject transactions whose fee rate is higher than the specified value, expressed in " + CURRENCY_UNIT +
                 "/kvB.\nFee rates larger than 1FJARCODE/kvB are rejected.\nSet to 0 to accept any fee rate."},
        ...
        RPCResult{
                                      ...
                                      "Transactions that cannot be fully validated due to failures in other transactions will not contain an 'allowed' result.\n",
            {
                {RPCResult::Type::OBJ, "", "",
                {
                    ...
                    {RPCResult::Type::STR_HEX, "wtxid", "The transaction witness hash in hex (same as txid after FJAR fork since witness data is not present)"},
                    {RPCResult::Type::STR, "package-error", /*optional=*/true, "Package validation error, if any (only possible if rawtxs had more than 1 transaction)."},
                    {RPCResult::Type::BOOL, "allowed", /*optional=*/true, "Whether this tx would be accepted to the mempool and pass client-specified maxfeerate. "
                                                       "If not present, the tx was not fully validated due to a failure in another tx in the list."},
                    {RPCResult::Type::NUM, "vsize", /*optional=*/true, "Virtual transaction size as defined in BIP 141. After FJAR fork, this equals the actual serialized size since witness data is not present (only present when 'allowed' is true)"},
...
{
    return {
        RPCResult{RPCResult::Type::NUM, "vsize", "virtual transaction size as defined in BIP 141. After FJAR fork, this equals the actual serialized size since witness data is not present."},
        RPCResult{RPCResult::Type::NUM, "weight", "transaction weight as defined in BIP 141. After FJAR fork, weight equals size * 4 since there is no witness data."},
        ...
        RPCResult{RPCResult::Type::STR_HEX, "wtxid", "hash of serialized transaction, including witness data (same as txid after FJAR fork)"},
