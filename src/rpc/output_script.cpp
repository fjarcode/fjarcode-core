...
static RPCHelpMan getcodequantumaddress()
{
    return RPCHelpMan{
        "getcodequantumaddress",
        "Build a Code Quantum cashaddr destination from a 32-byte hash.\n",
        {
            {"hash", RPCArg::Type::STR_HEX, RPCArg::Optional::NO, "32-byte hex hash for Code Quantum address payload"},
        },
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
                {RPCResult::Type::STR, "address", "The encoded Code Quantum cashaddr"},
                {RPCResult::Type::STR_HEX, "scriptPubKey", "The hex-encoded output script generated by the address"},
                {RPCResult::Type::BOOL, "isquantum", "Always true for this RPC"},
                {RPCResult::Type::STR, "quantum_type", "Code Quantum address subtype"},
                {RPCResult::Type::STR_HEX, "quantum_hash", "The 32-byte Code Quantum hash payload"},
            }
        },
        RPCExamples{
            HelpExampleCli("getcodequantumaddress", "\"11223344556677889900aabbccddeeff00112233445566778899aabbccddeeff\"") +
            HelpExampleRpc("getcodequantumaddress", "\"11223344556677889900aabbccddeeff00112233445566778899aabbccddeeff\"")
        },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            std::vector<unsigned char> hash = ParseHexV(request.params[0], "hash");
            if (hash.size() != 32) {
                throw JSONRPCError(RPC_INVALID_PARAMETER, "hash must be exactly 32 bytes");
            }

            QuantumHash quantum_hash;
            std::copy(hash.begin(), hash.end(), quantum_hash.begin());
            const CTxDestination dest{quantum_hash};
            const CScript script_pub_key = GetScriptForDestination(dest);

            UniValue result(UniValue::VOBJ);
            result.pushKV("address", EncodeDestination(dest));
            result.pushKV("scriptPubKey", HexStr(script_pub_key));
            result.pushKV("isquantum", true);
            result.pushKV("quantum_type", "code_quantum_cashaddr");
            result.pushKV("quantum_hash", HexStr(quantum_hash));
            return result;
        },
    };
}
