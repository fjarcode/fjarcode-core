...
void ParsePrevouts(const UniValue& prevTxsUnival, FillableSigningProvider* keystore, std::map<COutPoint, Coin>& coins)
{
    if (!prevTxsUnival.isNull()) {
        ...
        for (unsigned int idx = 0; idx < prevTxs.size(); ++idx) {
            ...
            // After FJAR fork, new witness transactions are not valid, but existing PSBTs may still need processing.
...
void SignTransaction(CMutableTransaction& mtx, const SigningProvider* keystore, const std::map<COutPoint, Coin>& coins, const UniValue& hashType, UniValue& result, int nCurrentHeight)
{
    int nHashType = ParseSighashString(hashType);

    // Since this is used by signrawtransactionwithkey which bypasses wallet,
    // we need to add FORKID here if not already present
    const Consensus::Params& consensusParams = Params().GetConsensus();
    // Use height+1: tx will be mined in the NEXT block, matching mempool behavior
    bool forkActive = (nCurrentHeight >= 0) ? consensusParams.IsFJARCODEActive(nCurrentHeight + 1)
                                            : (consensusParams.FJARCODEActivationHeight != Consensus::NEVER_ACTIVE_HEIGHT);
    if (forkActive) {
