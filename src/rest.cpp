...
// compatibility with historical block/transaction data. After FJAR fork, new
// transactions have no witness data.
...
static NodeContext* GetNodeContext(const std::any& context, HTTPRequest* req)
{
    ...
    if (!node_context) {
        ...
        RESTERR(req, HTTP_INTERNAL_SERVER_ERROR,
                "Internal error: Node context not found. "
...
static ChainstateManager* GetChainman(const std::any& context, HTTPRequest* req)
{
    ...
    if (!node_context || !node_context->chainman) {
        // to prevent information disclosure to potential attackers
        RESTERR(req, HTTP_INTERNAL_SERVER_ERROR,
                "Internal error: Chainman not available. "
                "Please report this issue if it persists.");
        return nullptr;
    }
                         ...
                         const std::string& strURIPart)
{
    ...
    headers.reserve(*parsed_count);
    {
        ...
        if (!tip) {
            return RESTERR(req, HTTP_SERVICE_UNAVAILABLE, "Blockchain not initialized");
        }
...
static bool rest_getutxos(const std::any& context, HTTPRequest* req, const std::string& strURIPart)
{
    ...
    if (param.length() > 1)
    {
        ...
        // MAX_GETUTXOS_OUTPOINTS * 75 (max chars per outpoint: 64 hex + '-' + 10 digits)
        static constexpr size_t MAX_URI_LENGTH = MAX_GETUTXOS_OUTPOINTS * 75 + 100;
        if (strUriParams.length() > MAX_URI_LENGTH) {
            return RESTERR(req, HTTP_BAD_REQUEST, "URI too long");
        }
        uriParts = SplitString(strUriParams, '/');
        if (uriParts.size() > MAX_GETUTXOS_OUTPOINTS + 1) {  // +1 for optional "checkmempool"
    ...
    if (uriParts.size() > 0)
    {
        ...
        {
            int32_t nOutput;
            size_t delimPos = uriParts[i].find('-');
            if (delimPos == std::string::npos) {
                return RESTERR(req, HTTP_BAD_REQUEST, "Parse error: missing output index delimiter");
            }
            std::string strTxid = uriParts[i].substr(0, delimPos);
            std::string strOutput = uriParts[i].substr(delimPos + 1);

            if (!ParseInt32(strOutput, &nOutput) || !IsHex(strTxid) || strTxid.size() != 64)
                return RESTERR(req, HTTP_BAD_REQUEST, "Parse error: invalid txid or output index");
    ...
    case RESTResponseFormat::BINARY: {
        try {
            ...
            if (strRequestMutable.size() > 0)
            {
                ...
                // Each outpoint is 36 bytes (32 txid + 4 index), plus 1 byte for checkMempool flag
                static constexpr size_t MAX_BINARY_SIZE = 1 + (MAX_GETUTXOS_OUTPOINTS * 36) + 100;
                if (strRequestMutable.size() > MAX_BINARY_SIZE) {
                    return RESTERR(req, HTTP_BAD_REQUEST, "Binary request too large");
                }
                       ...
                       const std::string& str_uri_part)
{
    ...
    int32_t blockheight = -1; // Initialization done only to prevent valgrind false positive, see https://github.com/fjarcode/fjarcode/pull/18785
