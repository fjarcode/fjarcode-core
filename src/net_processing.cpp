...
static const int MAX_NUM_UNCONNECTING_HEADERS_MSGS = 5;
...
private:
    ...
    */
    ...
    void RelayDSProof(const DoubleSpendProof& dsp);
...
// All FJAR peers can serve all data we need.
...
void PeerManagerImpl::FindNextBlocks(std::vector<const CBlockIndex*>& vBlocks, const Peer& peer, CNodeState *state, const CBlockIndex *pindexWalk, unsigned int count, int nWindowEnd, const CChain* activeChain, NodeId* nodeStaller)
{
    ...
    while (pindexWalk->nHeight < nMaxHeight) {
        ...
        for (const CBlockIndex* pindex : vToFetch) {
            ...
            if (!CanServeWitnesses(peer) && m_chainman.GetConsensus().IsSegwitActive(pindex->nHeight)) {
...
void PeerManagerImpl::AddToCompactExtraTransactions(const CTransactionRef& tx)
{
    ...
    // Each transaction can be up to MAX_STANDARD_TX_WEIGHT bytes
    // Cap at 5MB total to prevent memory exhaustion
    static constexpr size_t MAX_EXTRA_TXN_MEMORY = 5 * 1024 * 1024; // 5MB
    static constexpr size_t MAX_TX_SIZE_FOR_EXTRA = 100000; // 100KB per tx

    // Skip transactions that are too large for the extra buffer
    if (GetTransactionWeight(*tx) > MAX_TX_SIZE_FOR_EXTRA) {
        return;
    }
...
void PeerManagerImpl::NewPoWValidBlock(const CBlockIndex *pindex, const std::shared_ptr<const CBlock>& pblock)
{
    ...
    // FJAR: compact blocks work without witness data post-fork.
    // Only gate on SegWit for pre-fork blocks on the pre-fork chain.
    if (!m_chainman.GetConsensus().IsFJARCODEActive(pindex->nHeight) &&
        !m_chainman.GetConsensus().IsSegwitActive(pindex->nHeight)) return;
...
void PeerManagerImpl::ProcessGetData(CNode& pfrom, Peer& peer, const std::atomic<bool>& interruptMsgProc)
{
    ...
    while (it != peer.m_getdata_requests.end() && it->IsMsgDSProof()) {
        if (interruptMsgProc) return;
        if (pfrom.fPauseSend) break;
        ...
        if (DoubleSpendProof::IsEnabled()) {
            DoubleSpendProof dsp = m_mempool.doubleSpendProofStorage().lookup(inv.hash);
            if (!dsp.isEmpty()) {
                MakeAndPushMessage(pfrom, NetMsgType::DSPROOF, dsp);
            } else {
                vNotFound.push_back(inv);
            }
        }
...
void PeerManagerImpl::RelayDSProof(const DoubleSpendProof& dsp)
{
    if (!DoubleSpendProof::IsEnabled()) {
        return;
    }

    CInv inv(MSG_DOUBLESPENDPROOF, dsp.GetId());
    LogPrint(BCLog::NET, "Relaying DSProof INV: %s\n", dsp.GetId().ToString());

    m_connman.ForEachNode([&inv, this](CNode* pnode) {
        // Only relay to full relay peers (not block-relay-only)
        PeerRef peer = GetPeerRef(pnode->GetId());
        if (peer && peer->GetTxRelay()) {
            this->MakeAndPushMessage(*pnode, NetMsgType::INV, std::vector<CInv>{inv});
        }
    });
}
    ...
    if (CanDirectFetch() && last_header.IsValid(BLOCK_VALID_TREE) && m_chainman.ActiveChain().Tip()->nChainWork <= last_header.nChainWork) {
        ...
        while (pindexWalk && !m_chainman.ActiveChain().Contains(pindexWalk) && vToFetch.size() <= MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
            if (!(pindexWalk->nStatus & BLOCK_HAVE_DATA) &&
                    ...
                    (!m_chainman.GetConsensus().IsSegwitActive(pindexWalk->nHeight) || CanServeWitnesses(peer))) {
    ...
    while (CTransactionRef porphanTx = m_orphanage.GetTxToReconsider(peer.m_id)) {
        ...
        if (result.m_result_type == MempoolAcceptResult::ResultType::VALID) {
            ...
            if (state.GetResult() != TxValidationResult::TX_WITNESS_STRIPPED) {
                ...
                // https://github.com/fjarcode/fjarcode/issues/8279 for details.
                // We can remove this restriction (and always add wtxids to
                // the filter even for witness stripped transactions) once
                // wtxid-based relay is broadly deployed.
                // See also comments in https://github.com/fjarcode/fjarcode/pull/18044#discussion_r443419034
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::VERSION) {
        ...
        // This also prevents fingerprinting FJAR nodes via protocol negotiation
    ...
    if (msg_type == NetMsgType::SENDCMPCT) {
        ...
        // Since FJAR has no witness data, the version distinction is irrelevant.
        // v1 = non-witness, v2 = witness - both work identically on FJAR.
        if (sendcmpct_version != 1 && sendcmpct_version != 2) return;
    ...
    // We ignore incoming WTXIDRELAY messages but don't disconnect - the peer may be
    // running older software. All transactions are relayed by txid.
    if (msg_type == NetMsgType::WTXIDRELAY) {
        LogPrint(BCLog::NET, "FJAR: ignoring wtxidrelay from peer=%d (wtxid==txid on FJAR)\n", pfrom.GetId());
        return;
    }
    ...
    if (msg_type == NetMsgType::INV) {
        ...
        for (CInv& inv : vInv) {
            ...
            } else if (inv.IsMsgDSProof()) {
                if (DoubleSpendProof::IsEnabled()) {
                    // Check if we already have this proof
                    const bool fAlreadyHave = m_mempool.doubleSpendProofStorage().exists(inv.hash);
                    LogPrint(BCLog::NET, "got dsproof inv: %s  %s peer=%d\n", inv.hash.ToString(), fAlreadyHave ? "have" : "new", pfrom.GetId());
                    ...
                    LogPrint(BCLog::NET, "got dsproof inv (ignored, disabled): %s peer=%d\n", inv.hash.ToString(), pfrom.GetId());
                }
            } else {
    ...
    if (msg_type == NetMsgType::BLOCK)
    {
        ...
        int nNextHeight = prev_block ? prev_block->nHeight + 1 : 0;
        if (prev_block && IsBlockMutated(/*block=*/*pblock,
                           /*check_witness_root=*/m_chainman.GetConsensus().IsSegwitActive(nNextHeight))) {
    ...
    if (msg_type == NetMsgType::DSPROOF) {
        LogPrint(BCLog::NET, "Received a Double-Spend Proof from peer=%d\n", pfrom.GetId());
        if (!DoubleSpendProof::IsEnabled()) {
            // DSProof subsystem is disabled, ignore the message
            LogPrint(BCLog::NET, "DSProof subsystem is disabled, ignoring message\n");
            return;
        }
        ...
        try {
            ...
            LogPrint(BCLog::NET, "Failed to deserialize DSProof: %s\n", e.what());
            Misbehaving(*peer, 10, "bad-dsproof-deserialize");
            return;
        }

        // Check if we already have this proof
        if (m_mempool.doubleSpendProofStorage().exists(dsp.GetId())) {
            LogPrint(BCLog::NET, "DSProof already known: %s\n", dsp.GetId().ToString());
            return;
        }

        // Check if recently rejected
        if (m_mempool.doubleSpendProofStorage().isRecentlyRejectedProof(dsp.GetId())) {
            LogPrint(BCLog::NET, "DSProof recently rejected: %s\n", dsp.GetId().ToString());
            return;
        }
        ...
        DoubleSpendProof::Validity validity;
        {
            ...
            const CCoinsViewCache& coins_cache = m_chainman.ActiveChainstate().CoinsTip();
            validity = dsp.validate(m_mempool, &coins_cache);
        }

        switch (validity) {
            case DoubleSpendProof::Valid:
                // Add to storage
                if (m_mempool.doubleSpendProofStorage().add(dsp)) {
                    LogPrint(BCLog::NET, "DSProof accepted: %s for outpoint %s:%d\n",
                             dsp.GetId().ToString(),
                             dsp.prevTxId().ToString(),
                             dsp.prevOutIndex());
                    // Relay to peers
                    RelayDSProof(dsp);
                }
            ...
            case DoubleSpendProof::MissingTransaction:
                // Add as orphan - the spending tx might arrive later
                LogPrint(BCLog::NET, "DSProof is orphan (missing tx/utxo): %s\n", dsp.GetId().ToString());
                m_mempool.doubleSpendProofStorage().addOrphan(dsp, pfrom.GetId(), true);
                break;
            case DoubleSpendProof::Invalid:
                LogPrint(BCLog::NET, "DSProof invalid: %s\n", dsp.GetId().ToString());
                m_mempool.doubleSpendProofStorage().markProofRejected(dsp.GetId());
                Misbehaving(*peer, 10, "bad-dsproof");
                break;
        }
        return;
    }
