...
static uint64_t PolyMod(const std::vector<uint8_t>& v)
{
    uint64_t c = 1;
    for (uint8_t d : v) {
        uint8_t c0 = c >> 35;
...
static std::vector<uint8_t> ExpandPrefix(const std::string& prefix)
{
...
static std::vector<uint8_t> CreateChecksum(const std::string& prefix, const std::vector<uint8_t>& payload)
{
...
static bool VerifyChecksum(const std::string& prefix, const std::vector<uint8_t>& payload)
{
...
std::pair<std::string, std::vector<uint8_t>> Decode(const std::string& str, const std::string& default_prefix)
{
    ...
    std::transform(lower_prefix.begin(), lower_prefix.end(), lower_prefix.begin(), [](unsigned char c) {
...
std::pair<uint8_t, std::vector<uint8_t>> UnpackAddrData(const std::vector<uint8_t>& data)
{
    ...
    const uint8_t type = version >> 3;
    const uint8_t encoded_size = version & 0x07;
