...
// This is required to correctly serialize/deserialize historical blocks with witness data.
// After FJAR fork, new blocks have no witness data.
...
static constexpr uint8_t DB_FINALIZED_HEIGHT{'h'};
...
bool BlockTreeDB::WriteFinalizedHeight(int nHeight)
{
    return Write(DB_FINALIZED_HEIGHT, nHeight);
}

bool BlockTreeDB::ReadFinalizedHeight(int& nHeight)
{
    return Read(DB_FINALIZED_HEIGHT, nHeight);
}
