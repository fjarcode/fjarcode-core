...
{
    LOCK(m_lock);
    return m_recentRejects.contains({hash.begin(), hash.size()});
}

void DoubleSpendProofStorage::markProofRejected(const DspId &hash)
{
    LOCK(m_lock);
    m_recentRejects.insert({hash.begin(), hash.size()});
}
...
{
    // allow up to 25% more than maxOrphans() as a performance tweak, to avoid this being called for every orphan add.
    ...
    if (m_numOrphans > highWaterMark) {
        ...
        LogPrint(BCLog::MEMPOOL, "DSProof %s: reaped %d orphans, orphan count now %d (thresh-low: %d, thresh-high: %d\n",
