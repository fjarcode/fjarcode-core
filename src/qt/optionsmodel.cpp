...
/** Call node.updateRwSetting() with FJARCODE 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because FJARCODE 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/fjarcode/fjarcode/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because fjarcode 22.x will not try to read these.
...
//! Convert enabled/size values to fjarcode -prune setting.
...
//! Get pruning enabled value to show in GUI from fjarcode -prune setting.
...
//! Get pruning size value to show in GUI from fjarcode -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayFJARCODEUnit")) {
        settings.setValue("DisplayFJARCODEUnit", QVariant::fromValue(FJARCODEUnit::FJAR));
    }
    QVariant unit = settings.value("DisplayFJARCODEUnit");
    if (unit.canConvert<FJARCODEUnit>()) {
        m_display_fjarcode_unit = unit.value<FJARCODEUnit>();
    } else {
        m_display_fjarcode_unit = FJARCODEUnit::FJAR;
        settings.setValue("DisplayFJARCODEUnit", QVariant::fromValue(m_display_fjarcode_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if fjarcode.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_fjarcode_unit);
...
{
    if (new_unit.isNull() || new_unit.value<FJARCODEUnit>() == m_display_fjarcode_unit) return;
    m_display_fjarcode_unit = new_unit.value<FJARCODEUnit>();
    QSettings settings;
    settings.setValue("DisplayFJARCODEUnit", QVariant::fromValue(m_display_fjarcode_unit));
    Q_EMIT displayUnitChanged(m_display_fjarcode_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/fjarcode/fjarcode/pull/8273
    ...
    // (https://github.com/fjarcode-core/gui/issues/567).
