...
void PSBTOperationsDialog::openWithPSBT(PartiallySignedTransaction psbtx)
{
    ...
    int nCurrentHeight = m_client_model->node().getNumBlocks();
    ...
    bool complete = FinalizePSBT(psbtx, sighash_type); // Make sure all existing signatures are fully combined before checking for completeness.
    if (m_wallet_model) {
        size_t n_could_sign;
...
void PSBTOperationsDialog::broadcastTransaction()
{
    ...
    if (!FinalizeAndExtractPSBT(m_transaction_data, mtx, sighash_type)) {
...
void PSBTOperationsDialog::saveTransaction() {
    ...
    for (const CTxOut& out : m_transaction_data.tx->vout) {
        ...
        QString amount = FJARCODEUnits::format(m_client_model->getOptionsModel()->getDisplayUnit(), out.nValue);
...
QString PSBTOperationsDialog::renderTransaction(const PartiallySignedTransaction &psbtx)
{
    ...
    for (const CTxOut& out : psbtx.tx->vout) {
        ...
        tx_description.append(bullet_point).append(tr("Sends %1 to %2")
            .arg(FJARCODEUnits::formatWithUnit(FJARCODEUnit::FJAR, out.nValue))
    ...
    if (!*analysis.fee) {
        ...
        tx_description.append(FJARCODEUnits::formatWithUnit(FJARCODEUnit::FJAR, *analysis.fee));

        // add total amount in all subdivision units
        tx_description.append("<hr />");
        QStringList alternativeUnits;
        for (const FJARCODEUnits::Unit u : FJARCODEUnits::availableUnits())
        {
        ...
        tx_description.append(QString("<b>%1</b>: <b>%2</b>").arg(tr("Total Amount"))
            .arg(FJARCODEUnits::formatHtmlWithUnit(m_client_model->getOptionsModel()->getDisplayUnit(), totalAmount)));
