...
{
    GUIUtil::setClipboard(FJARCODEUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(CCoinControl& m_coin_control, WalletModel *model, QDialog* dialog)
{
    ...
    for (const auto& out : model->wallet().getCoins(vCoinControl)) {
        ...
        // After FJAR fork, new witness outputs cannot be created, but existing ones may still need to be spent.
    ...
    FJARCODEUnit nDisplayUnit = FJARCODEUnit::FJAR;
    ...
    l2->setText(FJARCODEUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(FJARCODEUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(FJARCODEUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l8->setText(FJARCODEUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0)
    {
...
void CoinControlDialog::updateView()
{
    ...
    FJARCODEUnit nDisplayUnit = model->getOptionsModel()->getDisplayUnit();
    ...
    for (const auto& coins : model->wallet().listCoins()) {
        ...
        for (const auto& outpair : coins.second) {
            ...
            if(ExtractDestination(out.txout.scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show FJAR address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, FJARCODEUnits::format(nDisplayUnit, out.txout.nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, FJARCODEUnits::format(nDisplayUnit, nSum));
