...
static constexpr const char* QT_UI_THEME_SETTING_KEY = "uiTheme";
static constexpr const char* QT_UI_THEME_DEFAULT = "darkred";
static constexpr const char* QT_UI_THEME_DARKRED = "darkred";
static constexpr const char* QT_UI_THEME_CLASSIC = "classic";

static void ApplyUiThemeFromSettings()
{

    QSettings settings;
    if (!settings.contains(QT_UI_THEME_SETTING_KEY)) {
        settings.setValue(QT_UI_THEME_SETTING_KEY, QT_UI_THEME_DEFAULT);
    }

    // Keep the UI consistently on darkred across platforms/releases.
    QString theme = QLatin1String(QT_UI_THEME_DARKRED);
    settings.setValue(QT_UI_THEME_SETTING_KEY, theme);
    ...
    palette.setColor(QPalette::Highlight, QColor(239, 68, 68));
    palette.setColor(QPalette::HighlightedText, QColor(255, 255, 255));
    palette.setColor(QPalette::Link, QColor(239, 68, 68));
    palette.setColor(QPalette::LinkVisited, QColor(248, 113, 113));
    ...
    if (auto* app = qobject_cast<QApplication*>(qApp)) app->setStyleSheet(QStringLiteral(
        "QMainWindow, QDialog, QTabWidget::pane, QStackedWidget, QScrollArea, QAbstractScrollArea {"
        "  background: #000000;"
        "  color: #f8fafc;"
        "}"
        "QWidget {"
        ...
        "  background: #111111;"
        "  color: #f8fafc;"
        "  selection-background-color: #f87171;"
        "  selection-color: #ffffff;"
        "}"
        "QPushButton {"
        "  border: 1px solid #2a2a2a;"
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in fjarcode.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in fjarcode.qrc)
...
int GuiMain(int argc, char* argv[])
{
    ...
    // Apply visual theme after network-specific app name is set, so QSettings lookup is chain-specific.
