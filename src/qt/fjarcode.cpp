...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<FjarcodeUnit>("FjarcodeUnit");
#else
    qRegisterMetaType<FjarcodeUnit>("FjarcodeUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. fjarcode_de.qm (shortcut "de" needs to be defined in fjarcode.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. fjarcode_de_DE.qm (shortcut "de_DE" needs to be defined in fjarcode.qrc)
...
static const char* qt_argv = "fjarcode-qt";

FjarcodeApplication::FjarcodeApplication()
...
void FjarcodeApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the FjarcodeApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", FjarcodeGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void FjarcodeApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool FjarcodeApplication::createOptionsModel(bool resetSettings)
{
    ...
    if (!optionsModel->Init(error)) {
        if (!resetSettings) {
            qWarning() << __func__ << ": GUI settings load failed, attempting automatic reset";
            optionsModel->Reset();
            bilingual_str retry_error;
            if (optionsModel->Init(retry_error)) {
                QMessageBox::warning(
                    nullptr,
                    PACKAGE_NAME,
                    tr("GUI settings were reset to defaults because the previous settings could not be loaded safely.")
                );
                return true;
            }
            error = retry_error;
...
void FjarcodeApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new FjarcodeGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &FjarcodeGUI::quitRequested, this, &FjarcodeApplication::requestShutdown);
...
void FjarcodeApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void FjarcodeApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool FjarcodeApplication::baseInitialize()
{
    return node().baseInitialize();
}

void FjarcodeApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &FjarcodeApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &FjarcodeApplication::handleRunawayException);
    connect(this, &FjarcodeApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &FjarcodeApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void FjarcodeApplication::parameterSetup()
{
...
void FjarcodeApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void FjarcodeApplication::requestInitialize()
{
...
void FjarcodeApplication::requestShutdown()
{
...
void FjarcodeApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // fjarcode: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &FjarcodeGUI::handlePaymentRequest);
            connect(window, &FjarcodeGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void FjarcodeApplication::handleRunawayException(const QString &message)
{
...
void FjarcodeApplication::handleNonFatalException(const QString& message)
{
...
WId FjarcodeApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool FjarcodeApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", FjarcodeGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(fjarcode);
    Q_INIT_RESOURCE(fjarcode_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(FJARCODE_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see fjarcode-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, PACKAGE_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see fjarcode-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse fjarcode.conf, determine network, switch to network specific
    ...
    // fjarcode: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
