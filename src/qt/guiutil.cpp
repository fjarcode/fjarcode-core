...
static QString ExampleAddress()
{
    return QStringLiteral("fjarcode:qqd70an6ctywm24hcf0fz9lac0kh8ws7vydmuflnyp");
}
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a FJARCODE address (e.g. %1)").arg(
        ExampleAddress()));
    widget->setValidator(new FJARCODEAddressEntryValidator(parent));
    widget->setCheckValidator(new FJARCODEAddressCheckValidator(parent));
}
...
bool parseFJARCODEURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!FJARCODEUnits::parse(FJARCODEUnit::FJAR, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
QString formatFJARCODEURI(const SendCoinsRecipient &info)
{
    ...
    // Strip CashAddr prefix if present to avoid double prefix (e.g. "fjarcode:fjarcode:q...")
    ...
    if (info.amount)
    {
        ret += QString("?amount=%1").arg(FJARCODEUnits::format(FJARCODEUnit::FJAR, info.amount, false, FJARCODEUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openFJARCODEConf()
{
    ...
    /* Open fjarcode.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "FJARCODE.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "FJARCODE (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("FJARCODE (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for FJARCODE*.lnk
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "fjarcode.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("fjarcode-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a fjarcode.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=FJARCODE\n";
        else
            optionFile << strprintf("Name=FJARCODE (%s)\n", ChainTypeToString(chain));
