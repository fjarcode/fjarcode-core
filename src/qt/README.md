This directory contains the source code for the FJARCODE Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the FJARCODE UI.
...
- ClientModel is used by the main application `fjarcodegui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a fjarcode:... link from a browser).
...
* UI elements like FJARCODEAmountField, which inherit from QWidget.
* `fjarcodestrings.cpp`: automatically generated
* `fjarcodeunits.(h/cpp)`: FJAR / mBC2 / etc. handling
* `callback.h`
...
**Note:** Do not change `local/fjarcode_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `fjarcode-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `fjarcode-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `fjarcode-qt`
9. You're all set! Start developing, building, and debugging the FJARCODE Core GUI
