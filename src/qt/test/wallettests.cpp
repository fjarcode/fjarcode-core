                  ...
                  QMessageBox::StandardButton confirm_type = QMessageBox::Yes)
{
    ...
    entry->findChild<FJARCODEAmountField*>("payAmount")->setValue(amount);
...
void CompareBalance(WalletModel& walletModel, CAmount expected_balance, QLabel* balance_label_to_check)
{
    ...
    QString balanceComparison = FJARCODEUnits::formatWithUnit(unit, expected_balance, false, FJARCODEUnits::SeparatorStyle::ALWAYS);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_fjarcode-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_fjarcode-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_fjarcode-qt  # macOS
void TestGUI(interfaces::Node& node, const std::shared_ptr<CWallet>& wallet)
{
    ...
    // Amount input
    FJARCODEAmountField* amountInput = receiveCoinsDialog.findChild<FJARCODEAmountField*>("reqAmount");
    amountInput->setValue(1);

    // Message input
    QLineEdit* messageInput = receiveCoinsDialog.findChild<QLineEdit*>("reqMessage");
    messageInput->setText("TEST_MESSAGE_1");
    QCOMPARE(addressType->currentData().toInt(), static_cast<int>(AddressFormat::CASHADDR));
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QCOMPARE(uri.count("fjarcode:"), 2);
            QCOMPARE(receiveRequestDialog->QObject::findChild<QLabel*>("address_tag")->text(), QString("Address:"));
            QVERIFY(address.isEmpty());
            address = receiveRequestDialog->QObject::findChild<QLabel*>("address_content")->text();
            QVERIFY(!address.isEmpty());
            QVERIFY(!address.startsWith("fjarcode:"));
    ...
    labelInput->setText("TEST_LABEL_2");
    amountInput->setValue(2);
    messageInput->setText("TEST_MESSAGE_2");
    const int cashaddr_index = addressType->findData(static_cast<int>(AddressFormat::CASHADDR));
    QVERIFY(cashaddr_index != -1);
    addressType->setCurrentIndex(cashaddr_index);
    requestPaymentButton->click();
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ReceiveRequestDialog* receiveRequestDialog = qobject_cast<ReceiveRequestDialog*>(widget);
            const QString candidate = receiveRequestDialog->QObject::findChild<QLabel*>("address_content")->text();
            if (candidate.startsWith("fjarcode:")) {
                cashaddr = candidate;
            }
        }
    }
    QVERIFY(!cashaddr.isEmpty());
