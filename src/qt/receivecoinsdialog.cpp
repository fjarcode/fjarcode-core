...
void ReceiveCoinsDialog::setModel(WalletModel *_model)
{
    ...
    if(_model && _model->getOptionsModel())
    {
        ...
        auto add_address_format = [&](AddressFormat format, const QString& text, const QString& tooltip) {
            const auto index = ui->addressType->count();
            ui->addressType->addItem(text, static_cast<int>(format));
            ui->addressType->setItemData(index, tooltip, Qt::ToolTipRole);
        };
        add_address_format(AddressFormat::CASHADDR, tr("CashAddr"), tr("FJARCODE CashAddr format with the fjarcode: prefix."));
        add_address_format(AddressFormat::LEGACY, tr("Base58 (Legacy)"), tr("Legacy P2PKH/P2SH address using Base58Check."));
        ui->addressType->setCurrentIndex(ui->addressType->findData(static_cast<int>(AddressFormat::CASHADDR)));
