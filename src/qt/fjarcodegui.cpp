    ...
    m_network_style(networkStyle)
{
    ...
    // In virtualized/remote sessions the tray path can be unstable on some Qt stacks.
    const bool disable_tray = qEnvironmentVariableIsSet("FJARCODE_QT_DISABLE_TRAY");

    // Create system tray icon and notification
    if (!disable_tray && QSystemTrayIcon::isSystemTrayAvailable()) {
        createTrayIcon();
    }
    ...
    {
        progressBar->setStyleSheet("QProgressBar { background-color: #111111; color: #f8fafc; border: 1px solid #2a2a2a; border-radius: 7px; padding: 1px; text-align: center; } QProgressBar::chunk { background: QLinearGradient(x1: 0, y1: 0, x2: 1, y2: 0, stop: 0 #ef4444, stop: 1 #f87171); border-radius: 7px; margin: 0px; }");
    }
    ...
    : m_platform_style{platformStyle}
{
    ...
    setStyleSheet(QString("QLabel { color : %1 }").arg(m_platform_style->TextColor().name()));
}
...
{
    if (e->type() == QEvent::PaletteChange) {
        QString style = QString("QLabel { color : %1 }").arg(m_platform_style->TextColor().name());
        if (style != styleSheet()) {
            setStyleSheet(style);
        }
    }
