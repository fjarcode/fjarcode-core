...
const std::string FjarcodeGUI::DEFAULT_UIPLATFORM =
...
FjarcodeGUI::FjarcodeGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &FjarcodeGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &FjarcodeGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &FjarcodeGUI::showModalOverlay);
...
void FjarcodeGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a FJARCODE address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and fjarcode: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &FjarcodeGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &FjarcodeGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &FjarcodeGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your FJARCODE addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified FJARCODE addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed FJARCODE Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed FJARCODE Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a fjarcode: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible FJARCODE command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &FjarcodeGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &FjarcodeGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &FjarcodeGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &FjarcodeGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &FjarcodeGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &FjarcodeGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &FjarcodeGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &FjarcodeGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &FjarcodeGUI::createWallet);
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_migrate_wallet_action, &QAction::triggered, [this] {
            auto activity = new MigrateWalletActivity(m_wallet_controller, this);
            connect(activity, &MigrateWalletActivity::migrated, this, &FjarcodeGUI::setCurrentWallet);
            activity->migrate(walletFrame->currentWalletModel());
        });
        connect(m_mask_values_action, &QAction::toggled, this, &FjarcodeGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &FjarcodeGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &FjarcodeGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &FjarcodeGUI::showDebugWindow);
}

void FjarcodeGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void FjarcodeGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &FjarcodeGUI::setCurrentWalletBySelectorIndex);
...
void FjarcodeGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &FjarcodeGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &FjarcodeGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &FjarcodeGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &FjarcodeGUI::showProgress);
...
#ifdef ENABLE_WALLET
void FjarcodeGUI::enableHistoryAction(bool privacy)
{
...
void FjarcodeGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &FjarcodeGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &FjarcodeGUI::removeWallet);
...
WalletController* FjarcodeGUI::getWalletController()
{
    return m_wallet_controller;
}

void FjarcodeGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &FjarcodeGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &FjarcodeGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &FjarcodeGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &FjarcodeGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &FjarcodeGUI::incomingTransaction);
    connect(this, &FjarcodeGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void FjarcodeGUI::removeWallet(WalletModel* walletModel)
{
...
void FjarcodeGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void FjarcodeGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void FjarcodeGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void FjarcodeGUI::setWalletActionsEnabled(bool enabled)
{
...
void FjarcodeGUI::createTrayIcon()
{
...
void FjarcodeGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &FjarcodeGUI::toggleHidden);
...
void FjarcodeGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void FjarcodeGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void FjarcodeGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void FjarcodeGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void FjarcodeGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void FjarcodeGUI::openClicked()
{
...
void FjarcodeGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void FjarcodeGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void FjarcodeGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void FjarcodeGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void FjarcodeGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void FjarcodeGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void FjarcodeGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void FjarcodeGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to FJARCODE network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void FjarcodeGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void FjarcodeGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void FjarcodeGUI::updateHeadersSyncProgressLabel()
{
...
void FjarcodeGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void FjarcodeGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &FjarcodeGUI::quitRequested);
...
void FjarcodeGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void FjarcodeGUI::createWallet()
{
    ...
    connect(activity, &CreateWalletActivity::created, this, &FjarcodeGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void FjarcodeGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void FjarcodeGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &FjarcodeGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &FjarcodeGUI::show);
                e->ignore();
            }
        }
    }
...
void FjarcodeGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void FjarcodeGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void FjarcodeGUI::incomingTransaction(const QString& date, FjarcodeUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(FjarcodeUnits::formatWithUnit(unit, amount, true));
...
void FjarcodeGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void FjarcodeGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool FjarcodeGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool FjarcodeGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void FjarcodeGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void FjarcodeGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void FjarcodeGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void FjarcodeGUI::updateProxyIcon()
{
    std::string ip_port;
...
void FjarcodeGUI::updateWindowTitle()
{
...
void FjarcodeGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void FjarcodeGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void FjarcodeGUI::detectShutdown()
{
...
void FjarcodeGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void FjarcodeGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(FjarcodeGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = FjarcodeGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void FjarcodeGUI::subscribeToCoreSignals()
{
...
void FjarcodeGUI::unsubscribeFromCoreSignals()
{
...
bool FjarcodeGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<FjarcodeUnit> units = FjarcodeUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const FjarcodeUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, FjarcodeUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const FjarcodeUnit u : FjarcodeUnits::availableUnits()) {
        menu->addAction(FjarcodeUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(FjarcodeUnit newUnits)
{
    setText(FjarcodeUnits::longName(newUnits));
}
