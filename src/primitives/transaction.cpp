...
CTxOut::CTxOut(const CAmount& nValueIn, CScript scriptPubKeyIn, const OutputToken& token)
{
    nValue = nValueIn;
    scriptPubKey = scriptPubKeyIn;
    tokenData.emplace(token);
}

std::string CTxOut::ToString() const
{
    std::string result = strprintf("CTxOut(nValue=%d.%08d, scriptPubKey=%s", nValue / COIN, nValue % COIN, HexStr(scriptPubKey).substr(0, 30));
    if (tokenData) {
        result += ", " + tokenData->ToString();
    }
    result += ")";
    return result;
}
...
// since witness data is not created. These functions remain for pre-fork compatibility.
...
// since transactions have no witness data (wtxid equals txid).
