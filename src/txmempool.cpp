...
void CTxMemPoolEntry::UpdateDescendantState(int32_t modifySize, CAmount modifyFee, int64_t modifyCount)
{
    ...
    CAmount oldFee = nModFeesWithDescendants;
    nModFeesWithDescendants = SaturatingAdd(nModFeesWithDescendants, modifyFee);
    if (modifyFee > 0 && nModFeesWithDescendants == oldFee) {
        LogPrintf("FJAR Warning: Fee saturation in descendant state (capped at INT64_MAX)\n");
    }
...
void CTxMemPoolEntry::UpdateAncestorState(int32_t modifySize, CAmount modifyFee, int64_t modifyCount, int64_t modifySigOps)
{
    ...
    CAmount oldFee = nModFeesWithAncestors;
    nModFeesWithAncestors = SaturatingAdd(nModFeesWithAncestors, modifyFee);
    if (modifyFee > 0 && nModFeesWithAncestors == oldFee) {
        LogPrintf("FJAR Warning: Fee saturation in ancestor state (capped at INT64_MAX)\n");
    }
...
void CTxMemPool::removeForBlock(const std::vector<CTransactionRef>& vtx, unsigned int nBlockHeight)
{
    ...
    // Notify DSProof storage that a new block was found
    // This clears the recently-rejected bloom filter and orphans existing proofs
    if (DoubleSpendProof::IsEnabled()) {
        m_dspStorage.newBlockFound();
    }
}
...
CTransactionRef CTxMemPool::addDoubleSpendProof(const DoubleSpendProof& proof)
{
    ...
    if (m_dspStorage.exists(proof.GetId())) {
        return nullptr;
    }

    // Find the transaction that spends this outpoint
    auto it = mapNextTx.find(proof.outPoint());
    if (it == mapNextTx.end()) {
        // No spending tx found, add as orphan
        m_dspStorage.addOrphan(proof, -1, true);
        return nullptr;
    }

    // Get the spending transaction
    CTransactionRef spendingTx = get(it->second->GetHash());
    if (!spendingTx) {
        return nullptr;
    }

    // Add the proof
    if (m_dspStorage.add(proof)) {
        LogPrint(BCLog::MEMPOOL, "DSProof added: %s for tx %s\n",
                 proof.GetId().ToString(), spendingTx->GetHash().ToString());
    }

    return spendingTx;
}

std::vector<CTxMemPool::DspTxIdPair> CTxMemPool::listDoubleSpendProofs(bool includeOrphans) const
{
    AssertLockHeld(cs);

    std::vector<DspTxIdPair> result;

    auto allProofs = m_dspStorage.getAll(includeOrphans);
    for (const auto& [proof, isOrphan] : allProofs) {
        ...
        if (!isOrphan) {
            ...
            auto it = mapNextTx.find(proof.outPoint());
            if (it != mapNextTx.end()) {
                txid = it->second->GetHash();
            }
        }
        result.emplace_back(proof, txid);
    }

    return result;
}

std::optional<CTxMemPool::DspTxIdPair> CTxMemPool::getDoubleSpendProof(const DspId& dspId) const
{
    AssertLockHeld(cs);

    DoubleSpendProof proof = m_dspStorage.lookup(dspId);
    if (proof.isEmpty()) {
        return std::nullopt;
    }
    ...
    return DspTxIdPair{proof, txid};
}

std::optional<CTxMemPool::DspTxIdPair> CTxMemPool::getDoubleSpendProofByOutpoint(const COutPoint& outpoint) const
{
    AssertLockHeld(cs);

    // Get all proofs and find one matching this outpoint
    auto allProofs = m_dspStorage.getAll(true);
    for (const auto& [proof, isOrphan] : allProofs) {
...
std::optional<DoubleSpendProof> CTxMemPool::getDoubleSpendProofByTxId(const uint256& txid) const
{
    AssertLockHeld(cs);
    ...
    // Check if any of the transaction's inputs have a DSProof
    auto allProofs = m_dspStorage.getAll(false);
    for (const auto& [proof, isOrphan] : allProofs) {
        for (const auto& vin : tx.vin) {
            if (vin.prevout == proof.outPoint()) {
                return proof;
            }
        }
    }
