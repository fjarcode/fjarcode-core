...
#ifdef USE_UPNP
static int CallUPNPGetValidIGD(
    int (*func)(UPNPDev*, UPNPUrls*, IGDdatas*, char*, int, char*, int),
    UPNPDev* devlist,
    UPNPUrls* urls,
    IGDdatas* data,
    char* lanaddr,
...
{
    char wanaddr[64];
    return func(devlist, urls, data, lanaddr, lanaddrlen, wanaddr, sizeof(wanaddr));
}

static int CallUPNPGetValidIGD(
    int (*func)(UPNPDev*, UPNPUrls*, IGDdatas*, char*, int),
    UPNPDev* devlist,
    UPNPUrls* urls,
    IGDdatas* data,
    char* lanaddr,
    int lanaddrlen)
{
    return func(devlist, urls, data, lanaddr, lanaddrlen);
}
#endif
...
static bool ProcessUpnp()
{
    ...
    r = CallUPNPGetValidIGD(UPNP_GetValidIGD, devlist, &urls, &data, lanaddr, sizeof(lanaddr));
    if (r == 1)
    {
        if (fDiscover) {
