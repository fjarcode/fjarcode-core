...
#ifndef FJARCODE_TXMEMPOOL_H
#define FJARCODE_TXMEMPOOL_H
...
public:
    ...
    //! Double-spend proof storage
    DoubleSpendProofStorage m_dspStorage;

    //! Access the double-spend proof storage (thread-safe methods)
    DoubleSpendProofStorage& doubleSpendProofStorage() { return m_dspStorage; }
    const DoubleSpendProofStorage& doubleSpendProofStorage() const { return m_dspStorage; }

    // -- FJAR: DSProof query types and methods --

    //! Result type: pair of (DoubleSpendProof, associated TxId or null)
    using DspTxIdPair = std::pair<DoubleSpendProof, uint256>;

    //! Add a double-spend proof and associate it with the spending transaction.
    //! @returns The spending transaction if found, nullptr otherwise.
    CTransactionRef addDoubleSpendProof(const DoubleSpendProof& proof) EXCLUSIVE_LOCKS_REQUIRED(cs);

    //! List all known proofs, optionally including orphans.
    //! @returns Vector of (proof, txid) pairs. Orphans have a null txid.
    std::vector<DspTxIdPair> listDoubleSpendProofs(bool includeOrphans = false) const EXCLUSIVE_LOCKS_REQUIRED(cs);

    //! Lookup a proof by DspId.
    //! @returns The proof and associated TxId if found.
    std::optional<DspTxIdPair> getDoubleSpendProof(const DspId& dspId) const EXCLUSIVE_LOCKS_REQUIRED(cs);

    //! Lookup a proof by the double-spent outpoint.
    //! @returns The proof and associated TxId if found.
    std::optional<DspTxIdPair> getDoubleSpendProofByOutpoint(const COutPoint& outpoint) const EXCLUSIVE_LOCKS_REQUIRED(cs);

    //! Lookup a proof by TxId (the spending transaction).
    //! @returns The proof if found for this transaction.
    std::optional<DoubleSpendProof> getDoubleSpendProofByTxId(const uint256& txid) const EXCLUSIVE_LOCKS_REQUIRED(cs);
