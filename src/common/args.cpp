...
const char * const FJARCODE_CONF_FILENAME = "fjarcode.conf";
const char * const FJARCODE_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //fjarcode-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", FJARCODE_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\FJARCODE
    // macOS: ~/Library/Application Support/FJARCODE
    // Unix-like: ~/.fjarcode
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "FJARCODE";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/FJARCODE";
#else
    // Unix-like
    return pathRet / ".fjarcode";
#endif
#endif
}
