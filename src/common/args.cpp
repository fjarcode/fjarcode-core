...
const char * const BITCOIN_CONF_FILENAME = "fjarcode.conf";
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\FJARCODE
    //   new: C:\Users\Username\AppData\Local\FJARCODE
    // macOS: ~/Library/Application Support/FJARCODE
    // Unix-like: ~/.fjarcode
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "FJARCODE";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "FJARCODE";
#else
    fs::path pathRet;
