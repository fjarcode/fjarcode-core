...
// After fork activation, wtxid always equals txid since witness data is not present.
bool TxOrphanage::AddTx(const CTransactionRef& tx, NodeId peer)
{
    ...
    auto& peer_count = m_peer_orphan_count[peer];
    if (peer_count >= MAX_ORPHANS_PER_PEER) {
        LogPrint(BCLog::TXPACKAGES, "ignoring orphan tx from peer %d (per-peer limit %u reached)\n", peer, MAX_ORPHANS_PER_PEER);
        return false;
    }
    ...
    LogPrint(BCLog::TXPACKAGES, "stored orphan tx %s (wtxid=%s) (mapsz %u outsz %u peer=%d peerorphans=%u)\n",
             hash.ToString(), wtxid.ToString(), m_orphans.size(), m_outpoint_to_orphan_it.size(), peer, peer_count);
    return true;
}
...
int TxOrphanage::EraseTxNoLock(const Txid& txid)
{
    ...
    auto peer_it = m_peer_orphan_count.find(peer);
...
void TxOrphanage::EraseForPeer(NodeId peer)
{
    ...
    m_peer_orphan_count.erase(peer);
