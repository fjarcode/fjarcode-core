...
static bool HTTPReq_JSONRPC(const std::any& context, HTTPRequest* req)
{
    ...
    if (!RPCAuthorized(authHeader.second, jreq.authUser)) {
        ...
        /* FJAR: Deter brute-forcing with variable delay
           Use random jitter to prevent timing-based attacks.
           If this results in a DoS the user really
           shouldn't have their RPC port exposed. */
        static thread_local FastRandomContext rng;
        // Random delay between 200-400ms to prevent timing attacks
        UninterruptibleSleep(std::chrono::milliseconds{200 + rng.randrange(200)});
