...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    // After FJAR fork, SegWit is disabled and witness data is not allowed
    ...
    if (consensusParams.IsFJARCODEActive(nextBlockHeight)) {
        if (tx.HasWitness()) {
            return state.Invalid(TxValidationResult::TX_CONSENSUS, "segwit-disabled",
                "SegWit transactions are not allowed after FJAR fork");
        }
    }
    ...
    // BCH Magnetic Anomaly reduced the minimum from 100 to 65 bytes; FJAR inherits this post-fork.
    const unsigned int minTxSize = consensusParams.IsFJARCODEActive(nextBlockHeight)
        ? FJARCODE_MIN_STANDARD_TX_SIZE
    ...
    for (const CTxIn &txin : tx.vin)
    {
        ...
        if (ptxConflicting) {
                ...
                if (DoubleSpendProof::IsEnabled()) {
                    try {
                        ...
                        if (m_active_chainstate.CoinsTip().GetCoin(txin.prevout, coin)) {
                            ...
                            const bool fjarcode_active_for_dsp = consensusParams.IsFJARCODEActive(nextBlockHeight);
                            const unsigned int dsp_flags2 = fjarcode_active_for_dsp
            ...
            if (!ws.m_conflicts.count(ptxConflicting->GetHash()))
            {
                ...
                // FJAR: RBF is disabled after fork — all transactions are first-seen-safe.
                // DSProof is used instead to notify merchants of double-spend attempts.
                const bool fjarcode_fork_active = consensusParams.IsFJARCODEActive(nextBlockHeight);
                const bool allow_rbf = fjarcode_fork_active ? false : (m_pool.m_full_rbf || SignalsOptInRBF(*ptxConflicting) || ptxConflicting->nVersion == 3);
                if (!allow_rbf) {
                    if (DoubleSpendProof::IsEnabled()) {
                        try {
                            ...
                            if (m_active_chainstate.CoinsTip().GetCoin(txin.prevout, coin)) {
                                ...
                                const unsigned int dsp_flags = fjarcode_fork_active
                                    ? FJARCODE_SCRIPT_VERIFY_FLAGS
                                    : STANDARD_SCRIPT_VERIFY_FLAGS;
                                auto dsproof = DoubleSpendProof::create(
                                    dsp_flags,
    ...
    // FJAR: Use fork-aware flags for sigop counting. Post-fork, we must NOT
    ...
    const unsigned int sigOpFlags = consensusParams.IsFJARCODEActive(nextBlockHeight)
        ? FJARCODE_SCRIPT_VERIFY_FLAGS
        : STANDARD_SCRIPT_VERIFY_FLAGS;
    ...
    // The only exception is v3 transactions (not applicable on FJAR post-fork).
    const bool is_v3_exempt = !m_active_chainstate.m_chainman.GetConsensus().IsFJARCODEActive(m_active_chainstate.m_chain.Height() + 1) && ws.m_ptx->nVersion == 3;
    ...
    if (!ancestors) {
        ...
        // this, see https://lists.linuxfoundation.org/pipermail/fjarcode-dev/2018-November/016518.html
    ...
    // FJAR: Skip V3/TRUC policy checks post-fork — FJAR has no V3 transaction concept
    if (!m_active_chainstate.m_chainman.GetConsensus().IsFJARCODEActive(m_active_chainstate.m_chain.Height() + 1)) {
...
bool MemPoolAccept::PolicyScriptChecks(const ATMPArgs& args, Workspace& ws)
{
    ...
    const Consensus::Params& consensusParams = m_active_chainstate.m_chainman.GetConsensus();
    const int nextBlockHeight = m_active_chainstate.m_chain.Height() + 1;
    const unsigned int scriptVerifyFlags = consensusParams.IsFJARCODEActive(nextBlockHeight)
        ? FJARCODE_SCRIPT_VERIFY_FLAGS
        : STANDARD_SCRIPT_VERIFY_FLAGS;
...
bool MemPoolAccept::ConsensusScriptChecks(const ATMPArgs& args, Workspace& ws)
{
    ...
    // FJAR: At the fork boundary, the tip is pre-fork but the next block needs
    // post-fork flags (SIGHASH_FORKID, NO_SEGWIT). Use next-block height to
    // determine the correct flags, otherwise valid FJAR txs are rejected.
    ...
    if (csParams.IsFJARCODEActive(nextHeight) && !csParams.IsFJARCODEActive(m_active_chainstate.m_chain.Height())) {
        // Fork boundary: tip is pre-fork, next block is post-fork
        currentBlockScriptVerifyFlags = FJARCODE_SCRIPT_VERIFY_FLAGS;
    } else {
...
static unsigned int GetBlockScriptFlags(const CBlockIndex& block_index, const ChainstateManager& chainman)
{
    ...
    if (consensusparams.IsFJARCODEActive(block_index.nHeight)) {
        // FJAR post-fork: P2SH enabled, SegWit/Taproot disabled, SIGHASH_FORKID required
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        int64_t nMaxSigOpsCost = params.GetConsensus().IsFJARCODEActive(pindex->nHeight) ? FJARCODE_MAX_BLOCK_SIGOPS_COST : PRE_FJARCODE_MAX_BLOCK_SIGOPS_COST;
...
void Chainstate::UpdateTip(const CBlockIndex* pindexNew)
{
    ...
    if (!m_chainman.IsInitialBlockDownload()) {
        ...
        for (int bit = 0; bit < VERSIONBITS_NUM_BITS; bit++) {
            ...
            // FJAR blocks signaled Taproot before the FJAR fork, triggering false warnings
...
bool Chainstate::ConnectTip(BlockValidationState& state, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions& disconnectpool)
{
    ...
    // FJAR/BCHN: Update automatic finalization (rolling checkpoints)
...
bool Chainstate::ActivateBestChainStep(BlockValidationState& state, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace)
{
    ...
    // FJAR/BCHN: Check if this reorg would cross the finalization boundary
    if (WouldCrossFinalization(pindexFork)) {
        // This reorg would disconnect finalized blocks - reject it
        LogPrintf("FJAR: Rejecting reorg that would cross finalization at height %d (fork at %d)\n",
...
// =========================================================================
// FJAR/BCHN Automatic Finalization (Rolling Checkpoints) Implementation
// =========================================================================
...
void Chainstate::UnparkBlock(CBlockIndex* pindex)
{
    ...
    LogPrintf("FJAR: Unparked block and descendants at height %d\n", nHeight);
}
...
void Chainstate::UpdateAutomaticFinalization(const CBlockIndex* pindexNew)
{
    ...
    // Only update finalization after FJAR fork is active
    if (!params.IsFJARCODEActive(pindexNew->nHeight)) {
        return;
    }
    ...
    if (finalizeHeight > m_finalized_height) {
        ...
        if (pindexFinalize && m_chain.Contains(pindexFinalize)) {
            ...
            LogPrintf("FJAR: Auto-finalized block at height %d (depth %d from tip %d)\n",
...
// =========================================================================
// End FJAR/BCHN Automatic Finalization
// =========================================================================
...
bool IsBlockMutated(const CBlock& block, bool check_witness_root)
{
    ...
    if (block.vtx.empty() || !block.vtx[0]->IsCoinBase()) {
        ...
        // https://lists.linuxfoundation.org/pipermail/fjarcode-dev/attachments/20190225/a27d8837/attachment-0001.pdf).
        //
...
static bool ContextualCheckBlock(const CBlock& block, BlockValidationState& state, const ChainstateManager& chainman, const CBlockIndex* pindexPrev)
{
    ...
    const Consensus::Params& consensusparams = chainman.GetConsensus();
    if (consensusparams.IsFJARCODEActive(nHeight)) {
        // After FJAR fork, no witness data is allowed
        for (const auto& tx : block.vtx) {
            if (tx->HasWitness()) {
                return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "unexpected-witness",
                    "SegWit transactions not allowed after FJAR fork");
            }
        }
        ...
        if (commitpos != NO_WITNESS_COMMITMENT) {
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "unexpected-witness-commitment",
                "Witness commitment not allowed after FJAR fork");
        }
    ...
    // ABLA's minimum limit equals the initial 32MB, so this is always >= FJARCODE_MAX_BLOCK_SIZE.
    ...
    if (consensusparams.IsFJARCODEActive(nHeight)) {
        ...
        nMaxBlockSize = PRE_FJARCODE_MAX_BLOCK_SIZE;
    }
...
bool ChainstateManager::ProcessNewBlock(const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, bool* new_block)
{
...

    {
        ...
        // https://lists.linuxfoundation.org/pipermail/fjarcode-dev/2019-February/016697.html.  Because CheckBlock() is
...
bool Chainstate::LoadChainTip()
{
    ...
    // FJAR: Restore finalization height on startup.
    ...
    if (csParams.maxReorgDepth > 0 && csParams.IsFJARCODEActive(tip->nHeight)) {
        ...
        if (finalizeHeight > 0) {
            m_finalized_height = finalizeHeight;
            LogPrintf("FJAR: Restored finalization height to %d on startup\n", m_finalized_height);
        }
    }
...
bool ChainstateManager::LoadBlockIndex()
{
    ...
    if (!fReindex) {
        ...
        for (CBlockIndex* pindex : vSortedByHeight) {
            ...
            // here.
            // FJAR/BCHN: Skip parked blocks when loading candidates
...
void ChainstateManager::CheckBlockIndex()
{
    ...
    CBlockIndex* pindexFirstParked = nullptr; // FJAR/BCHN: Oldest ancestor of pindex which is parked.
    while (pindex != nullptr) {
        nNodes++;
        ...
        // FJAR/BCHN: Parked blocks consistency check
        ...
        for (auto c : GetAll()) {
            ...
            if (!CBlockIndexWorkComparator()(pindex, c->m_chain.Tip()) && pindexFirstNeverProcessed == nullptr) {
                if (pindexFirstInvalid == nullptr) {
                    ...
                    // setBlockIndexCandidates or be parked (FJAR/BCHN).
                    //
                    if ((pindexFirstMissing == nullptr || pindex == c->m_chain.Tip())) {
                        ...
                        // FJAR/BCHN: Parked blocks are removed from candidates, so allow them.
