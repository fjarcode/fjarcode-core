...
// Copyright (c) 2017-2024 The Bitcoin Cash developers
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    bool fRescan = true;
    {
        ...
        CKeyID vchAddress = pubkey.GetID();
        {
            ...
            // FJAR: Determine fork status to filter witness destinations
            int nHeight = pwallet->GetLastBlockHeight();
            const Consensus::Params& consensusParams = Params().GetConsensus();
            bool forkActive = consensusParams.IsFJARCODEActive(nHeight);

            // We don't know which corresponding address will be used;
            // label all new addresses, and label existing addresses if a
            // label was passed.
            for (const auto& dest : GetAllDestinationsForKey(pubkey)) {
                if (forkActive && IsWitnessDestination(dest)) {
                    continue;  // Skip witness destinations post-fork
                }
            ...
            if (pubkey.IsCompressed() && !forkActive) {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
...

    {
        ...
        if (IsValidDestination(dest)) {
            ...
            int nHeight = pwallet->GetLastBlockHeight();
            const Consensus::Params& consensusParams = Params().GetConsensus();
            if (consensusParams.IsFJARCODEActive(nHeight) && IsWitnessDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Witness/SegWit addresses cannot be imported after FJAR fork");
            }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    bool fGood = true;
    {
        ...
        for (const auto& script_pair : scripts) {
            ...
            // FJAR: Skip importing witness scripts post-fork — they are unspendable
            if (Params().GetConsensus().IsFJARCODEActive(pwallet->GetLastBlockHeight())) {
                std::vector<std::vector<unsigned char>> solverdata;
                TxoutType type = Solver(script, solverdata);
                if (type == TxoutType::WITNESS_V0_KEYHASH ||
                    ...
                    pwallet->WalletLogPrintf("Skipping witness script import post-fork: %s\n", HexStr(script));
                    progress++;
                    continue;
                }
            }
...
RPCHelpMan dumpprivkey()
{
    return RPCHelpMan{"dumpprivkey",
                ...
                "Note: This command is only compatible with legacy wallets.\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The FJAR address for the private key"},
                },
                RPCResult{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
     ...
     * https://github.com/fjarcode/fjarcode/issues/9934
    ...
    file << strprintf("# * fork_id: FJAR (SIGHASH_FORKID=0x40)\n");
    file << "\n";
                    ...
                    result.pushKV(
                        ...
                        JSONRPCError(
                            ...
                            strprintf("Rescan failed for key with creation timestamp %d. There was an error reading a "
                                      ...
                                      "caused by pruning or data corruption (see fjarcoded log for details) and could "
...
static UniValue ProcessDescriptorImport(CWallet& wallet, const UniValue& data, const int64_t timestamp) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    try {
        ...
        bool is_witness_descriptor = false;
        bool fjarcode_fork_active = false;
        {
            LOCK(wallet.cs_wallet);
            int nHeight = wallet.GetLastBlockHeight();
            const Consensus::Params& consensusParams = Params().GetConsensus();
            fjarcode_fork_active = consensusParams.IsFJARCODEActive(nHeight);
            if (fjarcode_fork_active) {
                std::optional<OutputType> output_type = parsed_desc->GetOutputType();
                if (output_type && *output_type != OutputType::LEGACY) {
                    is_witness_descriptor = true;
                    // but they cannot be set as active (for new address generation)
                    if (active) {
                        throw JSONRPCError(RPC_INVALID_PARAMETER,
                            "Witness/SegWit descriptors cannot be set as active after FJAR fork. "
                            "Import without 'active: true' to claim/spend existing SegWit UTXOs.");
                    }
                    warnings.push_back("Importing SegWit descriptor for claiming existing UTXOs. "
                                       "New SegWit addresses cannot be generated after FJAR fork.");
                }
            }
        }
        ...
        if (active && !is_witness_descriptor) {
...
RPCHelpMan importdescriptors()
{
    return RPCHelpMan{"importdescriptors",
            ...
            "\nFJARCODE SEGWIT MIGRATION: After the FJAR fork, SegWit descriptors (wpkh, wsh, tr) can be imported for claiming\n"
            "existing UTXOs but cannot be set as active. The wallet will spend these UTXOs using scriptSig instead of witness.\n"
            "\nWARNING: Taproot script-path (P2TR with scripts) UTXOs are NOT supported for migration. Only Taproot key-path\n"
            "spending is supported. If you have Taproot script-path UTXOs, move them BEFORE the FJAR fork activates.\n",
                {
                    {"requests", RPCArg::Type::ARR, RPCArg::Optional::NO, "Data to be imported",
                        {
                            {"", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "",
