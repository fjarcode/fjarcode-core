    ...
    EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    // Add opt-in RBF status (always "no" post-fork since FJAR disables RBF)
    std::string rbfStatus = "no";
    if (confirms <= 0 && !Params().GetConsensus().IsFJARCODEActive(wallet.GetLastBlockHeight())) {
...
static std::vector<RPCResult> TransactionDescriptionString()
{
    return{{RPCResult::Type::NUM, "confirmations", "The number of confirmations for the transaction. Negative confirmations means the\n"
           ...
           {RPCResult::Type::STR_HEX, "wtxid", "The hash of serialized transaction, including witness data (same as txid after FJAR fork)."},
    ...
    return RPCHelpMan{"listtransactions",
                ...
                RPCResult{
                    RPCResult::Type::ARR, "", "",
                    {
                        ...
                        {
                            {RPCResult::Type::BOOL, "involvesWatchonly", /*optional=*/true, "Only returns true if imported addresses were involved in transaction."},
                            {RPCResult::Type::STR, "address",  /*optional=*/true, "The FJAR address of the transaction (not returned if the output does not have an address, e.g. OP_RETURN null data)."},
                ...
                RPCResult{
                    ...
                    TransactionDescriptionString()),
                    {
                        {RPCResult::Type::ARR, "details", "",
                        {
                            ...
                            {
                                {RPCResult::Type::BOOL, "involvesWatchonly", /*optional=*/true, "Only returns true if imported addresses were involved in transaction."},
                                {RPCResult::Type::STR, "address", /*optional=*/true, "The FJAR address involved in the transaction."},
                                {RPCResult::Type::STR, "category", "The transaction category.\n"
                                    "\"send\"                  Transactions sent.\n"
                                    "\"receive\"               Non-coinbase transactions received.\n"
                                    "\"generate\"              Coinbase transactions received with more than 100 confirmations.\n"
