    ...
    // after.
    {
        ...
        BOOST_CHECK_EQUAL(response.write(),
            strprintf("[{\"success\":false,\"error\":{\"code\":-1,\"message\":\"Rescan failed for key with creation "
                      ...
                      "by pruning or data corruption (see fjarcoded log for details) and could be dealt with by "
...
BOOST_FIXTURE_TEST_CASE(BasicOutputTypesTest, ListCoinsTest)
{
    ...
    for (const auto& out_type : OUTPUT_TYPES) {
        ...
        if (out_type == OutputType::P2SH_SEGWIT || out_type == OutputType::BECH32 || out_type == OutputType::BECH32M) continue;
...
{
    // With WITNESS_SCALE_FACTOR=1 (no witness discount), normal sig is 1 byte shorter than max sig.
    // The constant uses the max sig value for conservative fee estimation.
...
BOOST_FIXTURE_TEST_CASE(wallet_sync_tx_invalid_state_test, TestingSetup)
{
    ...
    // Add tx to wallet
    const auto op_dest{*Assert(wallet.GetNewDestination(OutputType::LEGACY, ""))};
