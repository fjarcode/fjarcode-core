...
// After FJAR fork, BECH32 (SegWit) addresses are not available for new transactions.
// Tests are retained for coin selection algorithm validation.
...
BOOST_AUTO_TEST_CASE(bnb_search_test)
{
    ...
    }
    {
        ...
        CAmount input_fee = coin_selection_params_bnb.m_effective_feerate.GetFee(/*num_bytes=*/148);
        ...
        // Add selectable outputs, increasing their raw amounts by their input fee to make the effective value equal to the raw amount
        input_fee = coin_selection_params_bnb.m_effective_feerate.GetFee(/*num_bytes=*/148);
        add_coin(available_coins, *wallet, 10 * CENT + input_fee, coin_selection_params_bnb.m_effective_feerate, 6 * 24, false, 0, true);
        add_coin(available_coins, *wallet, 9 * CENT + input_fee, coin_selection_params_bnb.m_effective_feerate, 6 * 24, false, 0, true);
        add_coin(available_coins, *wallet, 1 * CENT + input_fee, coin_selection_params_bnb.m_effective_feerate, 6 * 24, false, 0, true);
...
BOOST_AUTO_TEST_CASE(knapsack_solver_test)
{
    ...
    for (int i = 0; i < RUN_TESTS; i++)
    {
        ...
        BOOST_CHECK_EQUAL(result14->GetSelectedValue(), 1 * COIN);  // we should get 1 FJAR in 1 coin
        ...
        BOOST_CHECK_EQUAL(result15->GetSelectedValue(), 2 * COIN);  // we should get 2 FJAR in 1 coin
...
BOOST_AUTO_TEST_CASE(coin_grinder_tests)
{
...

    {
        ...
        // ###########################
        // With WITNESS_SCALE_FACTOR=1, each P2PKH input weighs ~148 bytes.
        // Need all 20 inputs (30 BTC) for 29.5 target, so weight ~2960.
        // Set max_weight=750 so it's clearly exceeded.
...

    {
        ...
        // Need ~27 inputs minimum, so 27 × 592 = ~16000 WU
        int max_weight = 20'000; // WU
        const auto& res = CoinGrinder(target, dummy_params, m_node, max_weight, [&](CWallet& wallet) {
            CoinsResult available_coins;
            for (int j = 0; j < 60; ++j) { // 60 UTXO --> 19.8 FJAR total
                add_coin(available_coins, wallet, CAmount(0.33 * COIN), CFeeRate(5000), 144, false, 0, true);
            }
            for (int i = 0; i < 10; i++) { // 10 UTXO --> 20 FJAR total
                add_coin(available_coins, wallet, CAmount(2 * COIN), CFeeRate(5000), 144, false, 0, true);
            }
            return available_coins;
        });
        BOOST_CHECK(res);
        if (res) {
            BOOST_CHECK(res->GetSelectionsEvaluated() > 0);
        }
    }

    {
        ...
        // 4) Test that two less valuable UTXOs with a combined lower weight are preferred over a more valuable heavier UTXO
        // #################################################################################################################
        // This test originally compared SegWit (68 bytes) vs Legacy (148 bytes) inputs
        // Now we just verify the algorithm finds a valid solution
        ...
        const auto& res = CoinGrinder(target, dummy_params, m_node, max_weight, [&](CWallet& wallet) {
            ...
            add_coin(available_coins, wallet, CAmount(1 * COIN), CFeeRate(5000), 144, false, 0, true, 148);
            add_coin(available_coins, wallet, CAmount(1 * COIN), CFeeRate(5000), 144, false, 0, true, 148);
            return available_coins;
        });
...

    {
        ...
        const auto& res = CoinGrinder(target, dummy_params, m_node, max_weight, [&](CWallet& wallet) {
            CoinsResult available_coins;
            // Expected Result: 4 + 3 + 2 + 1 = 10 FJAR at 400 vB
...
BOOST_AUTO_TEST_CASE(srd_tests)
{
...

    {
        ...
        // ###########################
        // With WITNESS_SCALE_FACTOR=1, each P2PKH input weighs ~148 bytes.
        // 20 inputs total weight ~2960. Set max_weight=750 to trigger weight exceeded.
        CAmount target = 49.5L * COIN;
        int max_weight = 750;
        const auto& res = SelectCoinsSRD(target, dummy_params, m_node, max_weight, [&](CWallet& wallet) {
            CoinsResult available_coins;
...
static util::Result<SelectionResult> select_coins(const CAmount& target, const CoinSelectionParams& cs_params, const CCoinControl& cc, std::function<CoinsResult(CWallet&)> coin_setup, const node::NodeContext& m_node)
{
    ...
    if (result) {
        const auto signedTxSize = 10 + 34 + 148 * result->GetInputSet().size(); // static header size + output size + inputs size (P2PKH)
...
BOOST_AUTO_TEST_CASE(check_max_weight)
{
    ...
    {
        // Scenario 1:
        // The actor starts with 1x 50.0 FJAR and 1515x 0.033 FJAR (~100.0 FJAR total) unspent outputs
        // Then tries to spend 49.5 FJAR
        // The 50.0 FJAR output should be selected, because the transaction would otherwise be too large
        ...
        BOOST_CHECK(result);
        // Verify that only the 50 FJAR UTXO was selected
    ...
    {
        // Scenario 2:
        // to avoid exceeding transaction weight limits. Using 1 FJAR and 0.5 FJAR UTXOs instead.

        // The actor starts with 100x 1.0 FJAR and 100x 0.5 FJAR (150.0 FJAR total) unspent outputs
        // Then tries to spend 49.5 FJAR
        ...
        const auto result = select_coins(
            target, cs_params, cc, [&](CWallet& wallet) {
                ...
                for (int j = 0; j < 100; ++j) {
                    add_coin(available_coins, wallet, CAmount(0.5 * COIN), CFeeRate(0), 144, false, 0, true);
                }
                return available_coins;
            },
            m_node);
        ...
        BOOST_CHECK(result);
        BOOST_CHECK(result->GetInputSet().size() > 0);
    }

    {
        // Scenario 3:

        // The actor starts with 1515x 0.033 FJAR (49.995 FJAR total) unspent outputs
...
BOOST_AUTO_TEST_CASE(SelectCoins_effective_value_test)
{
    ...
    CoinsResult available_coins;
    {
        ...
        add_coin(available_coins, *dummyWallet, 100000); // 0.001 FJAR
    }

    CAmount target{99900}; // 0.000999 FJAR
