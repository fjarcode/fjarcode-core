...
// Tests are retained for testing grouping algorithms which work with any output type.
...
BOOST_AUTO_TEST_CASE(outputs_grouping_tests)
{
    ...
    const CTxDestination dest = *Assert(wallet->GetNewDestination(OutputType::LEGACY, ""));
    for (unsigned long i = 0; i < GROUP_SIZE; i++) {
        addCoin(group_verifier.coins_pool, *wallet, dest, 10 * COIN, /*is_from_me=*/true);
    }

    group_verifier.GroupAndVerify(OutputType::LEGACY,
                                  BASIC_FILTER,
    ...
    const CTxDestination dest2 = *Assert(wallet->GetNewDestination(OutputType::LEGACY, ""));
    for (unsigned long i = 0; i < GROUP_SIZE; i++) {
    ...
    const CTxDestination dest3 = *Assert(wallet->GetNewDestination(OutputType::LEGACY, ""));
    addCoin(group_verifier.coins_pool, *wallet, dest3, 1, true, CFeeRate(100));
    BOOST_CHECK(group_verifier.coins_pool.coins[OutputType::LEGACY].back().GetEffectiveValue() <= 0);
    ...
    const CTxDestination dest4 = *Assert(wallet->GetNewDestination(OutputType::LEGACY, ""));
    ...
    const CTxDestination dest5 = *Assert(wallet->GetNewDestination(OutputType::LEGACY, ""));
    ...
    const CTxDestination dest7 = *Assert(wallet->GetNewDestination(OutputType::LEGACY, ""));
