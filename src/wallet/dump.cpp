...
namespace wallet {
static const std::string DUMP_MAGIC = "FJARCODE_CORE_WALLET_DUMP";
...
bool DumpWallet(const ArgsManager& args, WalletDatabase& db, bilingual_str& error)
{
    ...
    // Include fork version and current time to identify the fork era
    line = strprintf("%s,%s\n", "fork_id", "FJAR");
    dump_file.write(line.data(), line.size());
    hasher << Span{line};
...
// queue, which doesn't exist for the fjarcode-wallet. Define our own
// deleter here.
...
bool CreateFromDump(const ArgsManager& args, const std::string& name, const fs::path& wallet_path, bilingual_str& error, std::vector<bilingual_str>& warnings)
{
    ...
    if (ver != DUMP_VERSION) {
        error = strprintf(_("Error: Dumpfile version is not supported. This version of fjarcode-wallet only supports version 1 dumpfiles. Got dumpfile with version %s"), version_value);
        dump_file.close();
        return false;
    }
    ...
    // Handle optional metadata lines (fork_id, dump_time) that may appear
    // after the format line. These are included in the checksum but otherwise skipped.
    while (dump_file.good()) {
        auto pos = dump_file.tellg();
        std::string meta_key;
        std::getline(dump_file, meta_key, ',');
        std::string meta_value;
        std::getline(dump_file, meta_value, '\n');
        if (meta_key == "fork_id" || meta_key == "dump_time") {
            std::string meta_line = strprintf("%s,%s\n", meta_key, meta_value);
            hasher << Span{meta_line};
        } else {
            // Not a metadata line; seek back so the main loop can process it
            dump_file.seekg(pos);
            break;
        }
    }
