...
util::Result<CTxDestination> LegacyScriptPubKeyMan::GetNewDestination(const OutputType type)
{
    if (LEGACY_OUTPUT_TYPES.count(type) == 0) {
        return util::Error{_("Error: Only \"legacy\" address type is supported after FJAR fork")};
    }
...
{
    // instead of asserting to prevent wallet crashes during backup recovery
    if (type == OutputType::BECH32M) {
        LogPrintf("FJAR Warning: BECH32M (Taproot) address type requested but not supported post-fork\n");
        return;
    }
    // These scripts are needed to spend pre-fork witness outputs via scriptSig
    if (key.IsCompressed() && (type == OutputType::P2SH_SEGWIT || type == OutputType::BECH32)) {
        CTxDestination witdest = WitnessV0KeyHash(key.GetID());
        CScript witprog = GetScriptForDestination(witdest);
        // Make sure the resulting program is solvable.
        const auto desc = InferDescriptor(witprog, *this);
        if (desc && desc->IsSolvable()) {
            AddCScript(witprog);
        }
    }
}
...
std::unordered_set<CScript, SaltedSipHasher> LegacyScriptPubKeyMan::GetScriptPubKeys() const
{
    ...
    // It allows migrating wallets that were created before the FJAR fork.
    for (const auto& script_pair : mapScripts) {
        ...
        if (IsMine(script) == ISMINE_SPENDABLE) {
            ...
            // This allows wallet to find pre-fork witness UTXOs (P2WPKH, P2WSH, P2TR)
            int wit_ver = -1;
            std::vector<unsigned char> witprog;
            if (script.IsWitnessProgram(wit_ver, witprog)) {
                spks.insert(script);
                if (wit_ver == 1) {
                    LogPrintf("FJAR Warning: Found Taproot (witness v1) script. Only key-path spending is supported post-fork.\n");
                } else if (wit_ver > 1) {
                    LogPrintf("FJAR Warning: Found witness v%d script. This may not be spendable post-fork.\n", wit_ver);
                }
            }
        } else {
...
std::optional<MigrationData> LegacyScriptPubKeyMan::MigrateToDescriptor()
{
    ...
    // It allows migrating wallets that were created before the FJAR fork with SegWit multisigs.
    for (const auto& script_pair : mapScripts) {
        const CScript script = script_pair.second;
...
bool DescriptorScriptPubKeyMan::SetupDescriptorGeneration(WalletBatch& batch, const CExtKey& master_key, OutputType addr_type, bool internal)
{
    ...
    case OutputType::UNKNOWN: {
        // We should never reach here as SetupDescriptorScriptPubKeyMans only creates LEGACY
        assert(false);
    }
    } // no default case, so the compiler can warn about missing cases
    assert(!desc_prefix.empty());

    // FJAR uses coin type 145 (BCH standard) for mainnet, 1 for testnet/regtest
