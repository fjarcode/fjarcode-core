...
{
    // FJAR: RBF-based fee bumping is disabled after the fork. Use CPFP instead.
    const auto& params = Params().GetConsensus();
    int tipHeight = wallet.chain().getHeight().value_or(0);
    if (params.IsFJARCODEActive(tipHeight + 1)) {
        errors.push_back(Untranslated("Fee bumping via RBF is disabled on FJAR. Use child-pays-for-parent (CPFP) instead."));
...
bool SignTransaction(CWallet& wallet, CMutableTransaction& mtx) {
    ...
    if (wallet.IsWalletFlagSet(WALLET_FLAG_EXTERNAL_SIGNER)) {
        ...
        bool complete;
        wallet.FillPSBT(psbtx, complete, sighash_type, false /* sign */, true /* bip32derivs */);
        const TransactionError err = wallet.FillPSBT(psbtx, complete, sighash_type, true /* sign */, false  /* bip32derivs */);
