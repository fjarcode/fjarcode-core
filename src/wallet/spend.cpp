        ...
        bool sign) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    // Static vsize overhead + outputs vsize. 4 nVersion, 4 nLocktime, 1 input count (no witness overhead in FJAR)
    coin_selection_params.tx_noinputs_size = 9 + GetSizeOfCompactSize(vecSend.size()); // bytes for output count
    ...
    // FJAR: Force RBF signaling off post-fork — RBF is disabled, sequence should be MAX_SEQUENCE_NONFINAL
    bool signal_rbf = coin_control.m_signal_bip125_rbf.value_or(wallet.m_signal_rbf);
    if (signal_rbf && Params().GetConsensus().IsFJARCODEActive(wallet.GetLastBlockHeight())) {
        signal_rbf = false;
    }
    const uint32_t default_sequence{signal_rbf ? MAX_BIP125_RBF_SEQUENCE : CTxIn::MAX_SEQUENCE_NONFINAL};
