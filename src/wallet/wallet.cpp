...
bool CWallet::IsSpentKey(const CScript& scriptPubKey) const
{
    ...
    if (IsLegacy()) {
        ...
        // the avoid-reuse feature on wallets that had SegWit addresses before the fork.
...
CWalletTx* CWallet::AddToWallet(CTransactionRef tx, const TxState& state, const UpdateWalletTxFn& update_wtx, bool fFlushOnClose, bool rescanning_old_block)
{
    ...
    if (!strCmd.empty())
    {
        ...
        // https://github.com/fjarcode/fjarcode/pull/13339#issuecomment-537384875
        // A few ways it could be implemented in the future are described in:
        // https://github.com/fjarcode/fjarcode/pull/13339#issuecomment-461288094
...
bool CWallet::LoadToWallet(const uint256& hash, const UpdateWalletTxFn& fill_wtx)
{
    ...
    // If wallet doesn't have a chain (e.g when using fjarcode-wallet tool),
...
void CWallet::transactionRemovedFromMempool(const CTransactionRef& tx, MemPoolRemovalReason reason) {
    ...
    if (reason == MemPoolRemovalReason::CONFLICT) {
        ...
        //    https://github.com/fjarcode/fjarcode/pull/18600#discussion_r420195993.
        ...
        //    https://github.com/fjarcode/fjarcode/pull/9371 and the prior sync
        ...
        // https://github.com/fjarcode-core/fjarcode-devwiki/wiki/Wallet-Transaction-Conflict-Tracking
...
bool CWallet::SignTransaction(CMutableTransaction& tx) const
{
    ...
    // Check if wallet is initialized before calling GetLastBlockHeight()
    // Use height+1 because the tx will be mined in the NEXT block, matching mempool behavior
    int nHashType = SIGHASH_DEFAULT;
    const Consensus::Params& consensusParams = Params().GetConsensus();
    if (m_last_block_processed_height >= 0 && consensusParams.IsFJARCODEActive(m_last_block_processed_height + 1)) {
...
{
    // Check if wallet has been initialized with a block height before checking fork status
    // m_last_block_processed_height is -1 when uninitialized (e.g., in some test scenarios)
    // Use height+1 because the tx will be mined in the NEXT block
    const Consensus::Params& consensusParams = Params().GetConsensus();
    if (m_last_block_processed_height >= 0 && consensusParams.IsFJARCODEActive(m_last_block_processed_height + 1)) {
        // Add FORKID flag if not already set
        if ((sighash & SIGHASH_FORKID) == 0) {
            // Convert SIGHASH_DEFAULT to SIGHASH_ALL before adding FORKID
...
OutputType CWallet::TransactionChangeType(const std::optional<OutputType>& change_type, const std::vector<CRecipient>& vecSend) const
{
    ...
    // change addresses must always be legacy since SegWit is disabled.
    return OutputType::LEGACY;
}
...
{
    LOCK(cs_wallet);
    WalletLogPrintf("CommitTransaction:\n%s", tx->ToString()); // NOLINT(fjarcode-unterminated-logprintf)
 ...
 * https://fjarcodetalk.org/?topic=54527, or
 * https://github.com/fjarcode/fjarcode/pull/1393.
 */
    ...
    if (!gArgs.GetBoolArg("-walletcrosschain", DEFAULT_WALLETCROSSCHAIN)) {
        ...
        if (batch.ReadBestBlock(locator) && locator.vHave.size() > 0 && chain.getHeight()) {
            ...
            if (chain.getBlockHash(0) != locator.vHave.back()) {
                error = Untranslated("Wallet files should not be reused across chains. Restart fjarcoded with -walletcrosschain to override.");
                return false;
            }
        }
    }
...
void CWallet::SetupDescriptorScriptPubKeyMans(const CExtKey& master_key)
{
    ...
    // so we don't need P2SH_SEGWIT, BECH32, or BECH32M descriptor types.
    // This is true regardless of block height since FJAR is a new network.
    ...
    for (bool internal : {false, true}) {
        OutputType t = OutputType::LEGACY;
    ...
    if (!IsWalletFlagSet(WALLET_FLAG_EXTERNAL_SIGNER)) {
        ...
        for (bool internal : {false, true}) {
            ...
            for (const UniValue& desc_val : descriptor_vals.get_array().getValues()) {
                ...
                // FJAR: Only accept legacy (pkh) descriptors from external signers.
                // SegWit/Taproot descriptors would generate unspendable addresses post-fork.
                if (t != OutputType::LEGACY) {
                    WalletLogPrintf("Skipping non-legacy descriptor from external signer: %s\n", desc_str);
                    continue;
                }
