...
// and legacy compatibility. After FJAR fork, witness outputs cannot be created.
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " fjarcode-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "Usage:  fjarcode-tx [options] <hex-tx> [commands]  Update hex-encoded FJAR transaction\n"
                "or:     fjarcode-tx [options] -create [commands]   Create hex-encoded FJAR transaction\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded fjarcode transaction
