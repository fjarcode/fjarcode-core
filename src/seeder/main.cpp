...
// G_TRANSLATION_FUN is defined in seeder/translation.cpp
    ...
    int ParseCommandLine(int argc, char **argv) {
        ...
        if (HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
            ...
            if (HelpRequested(gArgs)) {
                strUsage +=
                    "\nUsage:  fjar-seeder -host=<host> -ns=<ns> "
        ...
        nThreads = gArgs.GetIntArg("-threads", DEFAULT_NUM_THREADS);
        nPort = gArgs.GetIntArg("-port", DEFAULT_PORT);
        nDnsThreads = gArgs.GetIntArg("-dnsthreads", DEFAULT_NUM_DNS_THREADS);
        ...
        if (gArgs.IsArgSet("-filter")) {
            ...
            while (flagstartpos < flagString.size()) {
                ...
                uint64_t flag = LocaleIndependentAtoi<uint64_t>(flagString.substr(
...
private:
    void SetupSeederArgs() {
        ...
        gArgs.AddArg("-testnet", "Use the test chain", ArgsManager::ALLOW_ANY,
                     OptionsCategory::OPTIONS);
        gArgs.AddArg("-signet", "Use the signet chain", ArgsManager::ALLOW_ANY,
                     OptionsCategory::OPTIONS);
        gArgs.AddArg("-regtest", "Use the regtest chain", ArgsManager::ALLOW_ANY,
                     OptionsCategory::OPTIONS);
    }
};
...
uint32_t CDnsThread::GetIPList(const char *requestedHostname, AddrGeneric *addr, uint32_t max, bool ipv4, bool ipv6) {
    ...
    } else if (strcasecmp(requestedHostname, this->host) != 0) {
        return 0;
    }
...
void SaveAllToDisk() {
    ...
    FILE *f = fsbridge::fopen("dnsseed.dat.new", "w+b");
    if (f) {
        try {
            AutoFile af(f);
            af << db;
            af.fclose();
            fs::rename("dnsseed.dat.new", "dnsseed.dat");
    ...
    for (const CAddrReport &rep : v) {
        std::fprintf(
            ...
            "  %5i \"%s\"\n",
            rep.ip.ToStringAddrPort().c_str(), rep.reliableness == Reliableness::OK ? 1 : 0, rep.lastSuccess,
...
void ThreadSeeder() {
    ...
    do {
        for (const std::string &seed : Params().DNSSeeds()) {
            ...
            std::vector<CNetAddr> ips = LookupHost(seed, MAX_HOSTS_PER_SEED, true);
            for (auto &ip : ips) {
                db.Add(CAddress(CService(ip, GetDefaultPort()), ServiceFlags(NODE_NETWORK)), true);
            }
        }
...
// Entry point called from seeder_main.cpp
int seeder_main(int argc, char **argv) {
    ...
    bool fDNS = true;
    std::fprintf(stdout, "Using %s.\n", gArgs.GetChainTypeString().c_str());
    if (opts.ns.empty()) {
    ...
    FILE *f = fsbridge::fopen("dnsseed.dat", "rb");
    if (f) {
        ...
        try {
            ...
            std::fprintf(stdout, "dnsseed.dat is either from a different version of this program or is corrupted.\n"
                                 "Please delete all data files to wipe the seeder database and restart.\n");
            return EXIT_FAILURE;
        }
    }
    ...
    if (opts.fReseed || dbStats.nAvail < 1) {
        ...
        // Parse the serialized fixed seeds data
        DataStream underlying_stream{Params().FixedSeeds()};
        ParamsStream s{CAddress::V2_NETWORK, underlying_stream};
        while (!s.eof()) {
            CService endpoint;
            s >> endpoint;
            db.Add(CAddress(endpoint, ServiceFlags(NODE_NETWORK)), true);
        }
    }
