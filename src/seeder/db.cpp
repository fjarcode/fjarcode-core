...
void CAddrDb::Add_(const CAddress &addr, bool force) {
    ...
    if (banned.count(ipp)) {
        ...
        if (force || (bantime < std::time(nullptr) && addrTimeSec > bantime)) {
            banned.erase(ipp);
        } else {
            return;
        }
    ...
    if (ipToId.count(ipp)) {
        ...
        auto addrTime = (addr.nTime == NodeSeconds{}) ? NodeSeconds{} : addr.nTime;
        auto addrTimeSec = std::chrono::duration_cast<std::chrono::seconds>(addrTime.time_since_epoch()).count();
        if (addrTimeSec > ai.lastTry) {
            ai.lastTry = addrTimeSec;
    ...
    auto addrTime = (addr.nTime == NodeSeconds{}) ? NodeSeconds{} : addr.nTime;
    ai.lastTry = std::chrono::duration_cast<std::chrono::seconds>(addrTime.time_since_epoch()).count();
    ai.ourLastTry = 0;
    ai.total = 0;
    ai.success = 0;
    int id = nId++;
