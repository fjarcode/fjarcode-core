...
BOOST_AUTO_TEST_CASE(TransactionsRequestDeserializationOverflowTest) {
    // Any set of index deltas that starts with N values that sum to (TYPE_MAX+1 - N)
    ...
    // FJAR: indexes are uint32_t (widened from uint16_t for 32MB blocks),
    // so the overflow boundary is 0x100000000 instead of 0x10000.
    BlockTransactionsRequest req0;
    req0.blockhash = InsecureRand256();
    req0.indexes.resize(3);
    req0.indexes[0] = 0x70000000;
    req0.indexes[1] = 0x100000000ULL - 0x70000000ULL - 2;
    req0.indexes[2] = 0;
    DataStream stream{};
