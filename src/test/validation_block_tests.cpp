...
// After FJAR fork, witness commitments are not required (SegWit is disabled).
    ...
    for (int i = 0; i < 10; i++) {
        threads.emplace_back([&]() {
            ...
            for (int i = 0; i < 1000; i++) {
                auto block = blocks[insecure.randrange(blocks.size())];
...
{
    // FJAR does not support SegWit (NO_SEGWIT flag). The mempool reorg locking
    // mechanism itself works correctly; this specific test just uses SegWit
    // transactions which are invalid in FJAR.
    return;
