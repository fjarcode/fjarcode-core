...
BasicTestingSetup::BasicTestingSetup(const ChainType chainType, const std::vector<const char*>& extra_args)
    : m_path_root{fs::temp_directory_path() / "test_common_" PACKAGE_NAME / g_insecure_rand_ctx_temp_path.rand256().ToString()},
      m_args{}
{
    // test didn't clean up properly (e.g., due to crash or exception)
    gArgs.ClearArgs();
    ...
    : TestingSetup{ChainType::REGTEST, extra_args, coins_db_in_memory, block_tree_db_in_memory}
{
    ...
    // Hash computed with mock time starting at 1750495500 (after genesis at 1750495460)
    {
        LOCK(::cs_main);
        // Print hash for debugging during development - remove once stabilized
        // LogPrintf("TestChain100Setup: Tip hash = %s\n", m_node.chainman->ActiveChain().Tip()->GetBlockHash().ToString());
    }
}
