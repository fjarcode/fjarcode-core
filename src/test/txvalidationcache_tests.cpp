...
// After FJAR fork, SCRIPT_VERIFY_WITNESS is disabled for new transactions.
// Tests are retained for validation cache behavior with historical blocks.
...
static constexpr int FJARCODE_SIGHASH_ALL = SIGHASH_ALL | SIGHASH_FORKID;
...
BOOST_FIXTURE_TEST_CASE(tx_mempool_block_doublespend, Dersig100Setup)
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        uint256 hash = SignatureHash(scriptPubKey, spends[i], 0, FJARCODE_SIGHASH_ALL, m_coinbase_txns[0]->vout[0].nValue, SigVersion::BCH_FORKID);
        BOOST_CHECK(coinbaseKey.Sign(hash, vchSig));
        vchSig.push_back((unsigned char)FJARCODE_SIGHASH_ALL);
...
static void ValidateCheckInputsForAllFlags(const CTransaction &tx, uint32_t failing_flags, bool add_to_cache, CCoinsViewCache& active_coins_tip) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    for (int count = 0; count < 10000; ++count) {
        ...
        // FJAR: flags use all 32 bits (up to 1U << 31), so the original
        // expression ((END_MARKER - 1) << 1) overflows to 0. Use rand32().
        uint32_t test_flags = insecure_rand.rand32();

        test_flags &= ~(SCRIPT_VERIFY_WITNESS | SCRIPT_VERIFY_WITNESS_PUBKEYTYPE | SCRIPT_VERIFY_CLEANSTACK);
...
BOOST_FIXTURE_TEST_CASE(checkinputs_test, Dersig100Setup)
{
    ...
    CScript p2wpkh_scriptPubKey = p2pkh_scriptPubKey;
    ...
    {
        std::vector<unsigned char> vchSig;
        uint256 hash = SignatureHash(p2pk_scriptPubKey, spend_tx, 0, FJARCODE_SIGHASH_ALL, m_coinbase_txns[0]->vout[0].nValue, SigVersion::BCH_FORKID);
        BOOST_CHECK(coinbaseKey.Sign(hash, vchSig));
    ...
    // Test CHECKLOCKTIMEVERIFY
    {
        ...
        // Sign
        std::vector<unsigned char> vchSig;
        uint256 hash = SignatureHash(spend_tx.vout[2].scriptPubKey, invalid_with_cltv_tx, 0, FJARCODE_SIGHASH_ALL, spend_tx.vout[2].nValue, SigVersion::BCH_FORKID);
        BOOST_CHECK(coinbaseKey.Sign(hash, vchSig));
        vchSig.push_back((unsigned char)FJARCODE_SIGHASH_ALL);
    ...
    // TEST CHECKSEQUENCEVERIFY
    {
        ...
        // Sign
        std::vector<unsigned char> vchSig;
        uint256 hash = SignatureHash(spend_tx.vout[3].scriptPubKey, invalid_with_csv_tx, 0, FJARCODE_SIGHASH_ALL, spend_tx.vout[3].nValue, SigVersion::BCH_FORKID);
        BOOST_CHECK(coinbaseKey.Sign(hash, vchSig));
        vchSig.push_back((unsigned char)FJARCODE_SIGHASH_ALL);
    ...
    // The witness tests have been removed as they are not applicable to FJAR.
    // The core validation cache functionality is still tested by the tests above.
}
