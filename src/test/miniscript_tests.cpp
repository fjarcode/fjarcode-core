...
// Miniscript parsing is retained for historical compatibility. After FJAR fork,
// new witness-based miniscripts cannot be used in transactions.
    ...
    for (int iter = 0; iter < 3; ++iter) {
        ...
        for (int add = -1; add < (int)challist.size(); ++add) {
            ...
            if (nonmal_success) {
                ...
                // (OP_CHECKSIGADD remapped to OP_CHECKDATASIG). Only verify P2WSH scripts.
                if (!miniscript::IsTapscript(converter.MsContext())) {
                    ScriptError serror;
...
BOOST_AUTO_TEST_CASE(fixed_tests)
{
    ...
    // its subs to all be 'u' (taken from https://github.com/rust-fjarcode/rust-miniscript/discussions/341).
