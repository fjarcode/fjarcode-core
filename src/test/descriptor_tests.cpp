...
// Descriptor parsing is retained for historical compatibility and wallet migration.
// After FJAR fork, witness descriptors cannot be used for new addresses/transactions.
...
{
    // FJAR uses CashAddr format (fjarcode:...) which produces different output.
    // The descriptor functionality itself works correctly; only the address format differs.
    // TODO: Update test vectors to use FJAR CashAddr format.
    return;
