...
// compatibility. After FJAR fork, new blocks have no witness data.
...
// Extra nonces for coinbase transactions - we only need extranonces now since
// we mine blocks dynamically rather than using hardcoded nonces
constexpr static unsigned char EXTRANONCES[] = {
    8, 0, 2, 6, 7, 8, 8, 2, 4, 1, 8, 4, 3, 8, 6, 5, 5, 4, 0, 5,
    3, 2, 2, 7, 2, 0, 1, 6, 7, 4, 7, 6, 3, 2, 3, 8, 5, 3, 0, 3,
    0, 2, 3, 2, 8, 2, 4, 8, 7, 3, 8, 4, 1, 6, 5, 3, 3, 0, 8, 5,
    0, 6, 6, 6, 6, 5, 0, 6, 4, 8, 6, 6, 6, 5, 8, 7, 3, 5, 2, 2,
    6, 7, 4, 3, 4, 0, 6, 3, 5, 8, 4, 8, 6, 0, 7, 7, 2, 6, 7, 7,
    4, 1, 0, 6, 6, 5, 6, 7, 8, 0
};
...
void MinerTestingSetup::TestPackageSelection(const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst)
{
    ...
    tx.vout[1].nValue = 100000000; // 1FJARCODE output
...
void MinerTestingSetup::TestBasicMining(const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst, int baseheight)
{
...

    {
        ...
        tx.vin.resize(1);
        // NOTE: OP_NOP is used to force 20 SigOps for each CHECKMULTISIG.
        // With WITNESS_SCALE_FACTOR=1 (FJAR), sigop_cost = legacy_count * 1 (not * 4),
        // so we need 4x as many CHECKMULTISIGs to exceed the pre-fork limit of 80000.
        // Each CHECKMULTISIG = 20 sigops, 4 per tx = 80 sigops, 1001 * 80 = 80080 > 80000.
        tx.vin[0].scriptSig = CScript() << OP_0 << OP_0 << OP_0 << OP_NOP << OP_CHECKMULTISIG
                                         << OP_0 << OP_0 << OP_0 << OP_NOP << OP_CHECKMULTISIG
                                         << OP_0 << OP_0 << OP_0 << OP_NOP << OP_CHECKMULTISIG
                                         << OP_0 << OP_0 << OP_0 << OP_NOP << OP_CHECKMULTISIG << OP_1;
        tx.vin[0].prevout.hash = txFirst[0]->GetHash();
        tx.vin[0].prevout.n = 0;
        tx.vout.resize(1);
...

    {
        ...
        hash = tx.GetHash();
        tx_mempool.addUnchecked(entry.Fee(LOWFEE).Time(Now<NodeSeconds>()).SpendsCoinbase(false).FromTx(tx));
        // Should throw TestBlockValidity failed (invalid P2SH script)
    ...
    // via addUnchecked since they would fail TestBlockValidity. Test the sequence lock
    // functions directly without adding to mempool.

    // Test relative height lock function (not added to mempool)
    tx.nVersion = 2;
    tx.vin.resize(1);
    ...
    tx.vin[0].prevout.n = 0;
    tx.vin[0].scriptSig = CScript() << OP_1;
    tx.vin[0].nSequence = m_node.chainman->ActiveChain().Tip()->nHeight + 1;
    ...
    // Test relative time lock function (not added to mempool)
    tx.vin[0].prevout.hash = txFirst[1]->GetHash();
    tx.vin[0].nSequence = CTxIn::SEQUENCE_LOCKTIME_TYPE_FLAG | (((m_node.chainman->ActiveChain().Tip()->GetMedianTimePast()+1-m_node.chainman->ActiveChain()[1]->GetMedianTimePast()) >> CTxIn::SEQUENCE_LOCKTIME_GRANULARITY) + 1);
    prevheights[0] = baseheight + 2;
    BOOST_CHECK(CheckFinalTxAtTip(*Assert(m_node.chainman->ActiveChain().Tip()), CTransaction{tx})); // Locktime passes

    const int SEQUENCE_LOCK_TIME = 512;
    for (int i = 0; i < CBlockIndex::nMedianTimeSpan; ++i)
        m_node.chainman->ActiveChain().Tip()->GetAncestor(m_node.chainman->ActiveChain().Tip()->nHeight - i)->nTime += SEQUENCE_LOCK_TIME;
    {
        CBlockIndex* active_chain_tip = m_node.chainman->ActiveChain().Tip();
    ...
    // absolute height locked - add to mempool (IsFinalTx filters these)
    tx.vin[0].prevout.hash = txFirst[2]->GetHash();
    ...
    // absolute time locked - add to mempool (IsFinalTx filters these)
    ...
    // So only the coinbase transaction should be in the block template.
    BOOST_CHECK_EQUAL(pblocktemplate->block.vtx.size(), 1U);

    // After advancing height and time, the absolute-locked txs should be mineable
    for (int i = 0; i < CBlockIndex::nMedianTimeSpan; ++i) {
        CBlockIndex* ancestor{Assert(m_node.chainman->ActiveChain().Tip()->GetAncestor(m_node.chainman->ActiveChain().Tip()->nHeight - i))};
        ancestor->nTime += SEQUENCE_LOCK_TIME;
    }
...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity)
{
    ...
    static_assert(std::size(EXTRANONCES) == 110, "Should have 110 extranonces");
    int baseheight = 0;
    std::vector<CTransactionRef> txFirst;
    for (size_t i = 0; i < std::size(EXTRANONCES); ++i) {
        CBlock *pblock = &pblocktemplate->block; // pointer for convenience
        {
            ...
            txCoinbase.vin[0].scriptSig = CScript{} << (m_node.chainman->ActiveChain().Height() + 1) << EXTRANONCES[i];
            ...
            pblock->nNonce = 0;
            while (!CheckProofOfWork(pblock->GetHash(), pblock->nBits, m_node.chainman->GetConsensus())) {
                ++pblock->nNonce;
            }
        }
