...
void MinerTestingSetup::TestPackageSelection(const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst)
{
    ...
    // There should be no fee-improving package inclusion from these txs.
    // Some chain/config contexts may still emit a refreshed template object,
    // so keep the behavioral assertion below as the source of truth.
    if (auto maybe_updated = block_template->waitNext({.timeout = MillisecondsDouble{0}, .fee_threshold = 1})) {
        block_template = std::move(maybe_updated);
    }
...
void MinerTestingSetup::TestBasicMining(const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst, int baseheight)
{
    ...
    if (Assert(m_node.chainman)->GetParams().GetChainType() == ChainType::REGTEST) {
        // Under regtest deployment state, nonfinal relative-locked mempool txs
        // can cause template creation to fail immediately.
...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity)
{
    ...
    // We can't make transactions until we have inputs, and later subtests
    // depend on mature coinbase outputs with empty scriptPubKey.
    constexpr int kBootstrapBlocks = 110;
    constexpr uint32_t kMaxNonceTriesPerBlock = 5'000'000;
    int baseheight = 0;
    std::vector<CTransactionRef> txFirst;

    for (int block_offset = 0; block_offset < kBootstrapBlocks; ++block_offset) {
        ...
        CMutableTransaction txCoinbase(*block.vtx[0]);
        {
            ...
            txCoinbase.vin[0].scriptSig = CScript{} << (current_height + 1) << block_offset;
        ...
        bool solved{false};
        for (uint32_t tries = 0; tries < kMaxNonceTriesPerBlock; ++tries) {
            if (CheckProofOfWork(block.GetHash(), block.nBits, Assert(m_node.chainman)->GetParams().GetConsensus())) {
                solved = true;
                break;
            }
            ++block.nNonce;
            BOOST_REQUIRE_NE(block.nNonce, 0U);
        }
        BOOST_REQUIRE_MESSAGE(solved, strprintf("Unable to find valid PoW nonce within %u tries at height=%d", kMaxNonceTriesPerBlock, current_height + 1));
        ...
        if (current_height % 2 == 0) {
            ...
            const bool accepted = Assert(m_node.chainman)->ProcessNewBlock(shared_pblock, /*force_processing=*/true, /*min_pow_checked=*/true, &new_block);
            if (!accepted) {
                BlockValidationState state;
                CBlockIndex* pindex{nullptr};
                bool accepted_by_acceptblock{false};
                {
                    LOCK(cs_main);
                    accepted_by_acceptblock = Assert(m_node.chainman)->AcceptBlock(shared_pblock, state, &pindex, /*fRequested=*/true, /*dbp=*/nullptr, /*fNewBlock=*/nullptr, /*min_pow_checked=*/true);
                }
                BOOST_REQUIRE_MESSAGE(false,
                    strprintf("ProcessNewBlock rejected new_block=%d acceptblock=%d reject=%s debug=%s",
                              new_block,
                              accepted_by_acceptblock,
