...
BOOST_AUTO_TEST_CASE(key_io_valid_gen)
{
    ...
    for (unsigned int idx = 0; idx < tests.size(); idx++) {
        ...
        if (isPrivkey) {
            ...
            // because FJAR uses CashAddr format while test vectors use base58
            CTxDestination expected_dest = DecodeDestination(exp_base58string);
            CTxDestination generated_dest = DecodeDestination(address);
            BOOST_CHECK_MESSAGE(IsValidDestination(expected_dest), "Cannot decode expected address: " + exp_base58string);
            BOOST_CHECK_MESSAGE(IsValidDestination(generated_dest), "Cannot decode generated address: " + address);
            BOOST_CHECK_MESSAGE(expected_dest == generated_dest, "Destination mismatch for " + exp_base58string + " vs " + address);
        }
    }
