...
BOOST_AUTO_TEST_CASE(cnetaddr_unserialize_v2)
{
    ...
    s << Span{ParseHex(
        ...
        "10"                                  // address length
        "fd6b88c08724ca978112ca1bbdcafac2")}; // address: 0xfd + sha256("fjarcode")[0:5] +
                                              // sha256(name)[0:10]
    s >> ser_params(addr);
...
BOOST_AUTO_TEST_CASE(get_local_addr_for_peer_port)
{
    ...
    CNode peer_out{/*id=*/0,
                   /*sock=*/nullptr,
                   /*addrIn=*/CAddress{CService{peer_out_in_addr, 8338}, NODE_NETWORK},
                   /*nKeyedNetGroupIn=*/0,
                   /*nLocalHostNonceIn=*/0,
                   /*addrBindIn=*/CAddress{},
    ...
    // Without the fix peer_us:8338 is chosen instead of the proper peer_us:bind_port.
    ...
    CNode peer_in{/*id=*/0,
                  /*sock=*/nullptr,
                  /*addrIn=*/CAddress{CService{peer_in_in_addr, 8338}, NODE_NETWORK},
                  /*nKeyedNetGroupIn=*/0,
                  /*nLocalHostNonceIn=*/0,
                  /*addrBindIn=*/CAddress{},
                  /*addrNameIn=*/std::string{},
    ...
    // Without the fix peer_us:8338 is chosen instead of the proper peer_us:peer_us.GetPort().
...
BOOST_AUTO_TEST_CASE(initial_advertise_from_version_message)
{
    ...
    CNode peer{/*id=*/0,
               /*sock=*/nullptr,
               /*addrIn=*/CAddress{CService{peer_in_addr, 8338}, NODE_NETWORK},
               /*nKeyedNetGroupIn=*/0,
               /*nLocalHostNonceIn=*/0,
               /*addrBindIn=*/CAddress{},
               /*addrNameIn=*/std::string{},
...
BOOST_AUTO_TEST_CASE(advertise_local_address)
{
    ...
    CAddress addr_ipv4{Lookup("1.2.3.4", 8338, false).value(), NODE_NONE};
    BOOST_REQUIRE(addr_ipv4.IsValid());
    BOOST_REQUIRE(addr_ipv4.IsIPv4());

    CAddress addr_ipv6{Lookup("1122:3344:5566:7788:9900:aabb:ccdd:eeff", 8338, false).value(), NODE_NONE};
    BOOST_REQUIRE(addr_ipv6.IsValid());
    BOOST_REQUIRE(addr_ipv6.IsIPv6());

    CAddress addr_ipv6_tunnel{Lookup("2002:3344:5566:7788:9900:aabb:ccdd:eeff", 8338, false).value(), NODE_NONE};
    BOOST_REQUIRE(addr_ipv6_tunnel.IsValid());
    BOOST_REQUIRE(addr_ipv6_tunnel.IsIPv6());
    BOOST_REQUIRE(addr_ipv6_tunnel.IsRFC3964());

    CAddress addr_teredo{Lookup("2001:0000:5566:7788:9900:aabb:ccdd:eeff", 8338, false).value(), NODE_NONE};
    ...
    CService service_cjdns{Lookup("fc00:3344:5566:7788:9900:aabb:ccdd:eeff", 8338, false).value(), NODE_NONE};
...
BOOST_AUTO_TEST_CASE(v2transport_test)
{
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        // FJAR: MAX_PROTOCOL_MESSAGE_LENGTH is 33MB (for 32MB blocks), so
        // the test payload must exceed that to trigger rejection.
        // Use zero-filled vector instead of randbytes to avoid slow RNG for 33MB.
        std::vector<uint8_t> msg_data_3(MAX_PROTOCOL_MESSAGE_LENGTH + 5000, 0);
