...
BOOST_AUTO_TEST_CASE(test_addnode_getaddednodeinfo_and_connection_detection)
{
    ...
    const std::string port_str = strprintf("%u", default_port);

    // Connect a localhost peer.
    {
        ASSERT_DEBUG_LOG(strprintf("Added connection to 127.0.0.1:%s peer=1", port_str));
    ...
    for (int i = 0; i < 10; ++i) {
        ASSERT_DEBUG_LOG(strprintf("Not opening a connection to localhost, already connected to 127.0.0.1:%s", port_str));
