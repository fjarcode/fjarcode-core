...
// After FJAR fork, new transactions do not include witness data.
...
static std::map<std::string, unsigned int> mapFlagNames = {
    ...
    // FJAR flags
    {std::string("SIGHASH_FORKID"), (unsigned int)SCRIPT_VERIFY_SIGHASH_FORKID},
    {std::string("NO_SEGWIT"), (unsigned int)SCRIPT_VERIFY_NO_SEGWIT},
};
...
// Get the mask of all known flags (flags that are in mapFlagNames)
// Excludes FJAR-specific flags that would break legacy test compatibility
unsigned int GetAllKnownFlags()
{
    unsigned int all_flags = 0;
    for (const auto& pair : mapFlagNames) {
        all_flags |= pair.second;
    }
    // Exclude FJAR-specific flags from the "all known" mask
    // These flags should only be enabled when explicitly requested in tests
    all_flags &= ~(SCRIPT_VERIFY_SIGHASH_FORKID | SCRIPT_VERIFY_NO_SEGWIT);
    return all_flags;
}

// Invert flags but only within the set of known flags
// This prevents FJAR-specific flags from being accidentally enabled
unsigned int InvertKnownFlags(unsigned int flags)
{
    static const unsigned int all_known = GetAllKnownFlags();
    return (~flags) & all_known;
}
...
BOOST_AUTO_TEST_CASE(tx_valid)
{
    ...
    for (unsigned int idx = 0; idx < tests.size(); idx++) {
        ...
        if (test[0].isArray())
        {
            ...
            unsigned int inverted_flags = InvertKnownFlags(verify_flags);
            if (inverted_flags != FillFlags(inverted_flags)) {
                BOOST_ERROR("Bad test flags: " << strTest);
            }

            BOOST_CHECK_MESSAGE(CheckTxScripts(tx, mapprevOutScriptPubKeys, mapprevOutValues, inverted_flags, txdata, strTest, /*expect_valid=*/true),
                                "Tx unexpectedly failed: " << strTest);

            // Backwards compatibility of script verification flags: Removing any flag(s) should not invalidate a valid transaction
            for (const auto& [name, flag] : mapFlagNames) {
                // Removing individual flags
                unsigned int flags = TrimFlags(InvertKnownFlags(verify_flags | flag));
                if (!CheckTxScripts(tx, mapprevOutScriptPubKeys, mapprevOutValues, flags, txdata, strTest, /*expect_valid=*/true)) {
                    BOOST_ERROR("Tx unexpectedly failed with flag " << name << " unset: " << strTest);
                }
                // Removing random combinations of flags
                flags = TrimFlags(InvertKnownFlags(verify_flags | (unsigned int)InsecureRandBits(mapFlagNames.size())));
                if (!CheckTxScripts(tx, mapprevOutScriptPubKeys, mapprevOutValues, flags, txdata, strTest, /*expect_valid=*/true)) {
            ...
            for (auto flags_excluding_one : ExcludeIndividualFlags(verify_flags)) {
                if (!CheckTxScripts(tx, mapprevOutScriptPubKeys, mapprevOutValues, InvertKnownFlags(flags_excluding_one), txdata, strTest, /*expect_valid=*/false)) {
...
BOOST_AUTO_TEST_CASE(test_IsStandard)
{
    ...
    // FJAR: MAX_OP_RETURN_RELAY=223, so 220 bytes of data + OP_RETURN + OP_PUSHDATA1 + len = 223
    t.vout[0].scriptPubKey = CScript() << OP_RETURN << ParseHex("");
    BOOST_CHECK_EQUAL(MAX_OP_RETURN_RELAY, t.vout[0].scriptPubKey.size());
    CheckIsStandard(t);

    // MAX_OP_RETURN_RELAY+1-byte TxoutType::NULL_DATA (non-standard)
    t.vout[0].scriptPubKey = CScript() << OP_RETURN << ParseHex("");
    ...
    // tx header:                12 bytes
    // 2438 inputs: 2438*41 = 99958 bytes
    //    1 output:              30 bytes
    //                      =============
    //                  total: 100000 bytes (= weight with WITNESS_SCALE_FACTOR=1)
    BOOST_CHECK_EQUAL(GetTransactionWeight(CTransaction(t)), 100000);
    CheckIsStandard(t);

    // increase output size by one byte, so we end up with 100001 weight
    t.vout[0].scriptPubKey = CScript() << OP_RETURN << std::vector<unsigned char>(20, 0); // output size: 31 bytes
    BOOST_CHECK_EQUAL(GetTransactionWeight(CTransaction(t)), 100001);
    ...
    // With WITNESS_SCALE_FACTOR=1 (no SegWit), no witness discount: input cost = 148 bytes
    // P2WPKH output = 31 bytes, total = 31 + 148 = 179
    ...
    // P2WSH output = 43 bytes, total = 43 + 148 = 191
    ...
    // P2TR output = 43 bytes, total = 43 + 148 = 191
    ...
    // Future witness output (OP_n + 2 bytes) = 13 bytes, total = 13 + 148 = 161
