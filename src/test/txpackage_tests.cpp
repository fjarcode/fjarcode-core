...
// After FJAR fork, witness data is not present in transactions.
// Tests are retained for pre-fork transaction relay compatibility.
...
{
    // FJAR does not support SegWit (NO_SEGWIT flag). Package validation functionality
    // is tested by the other tests in this file that don't rely on witness data.
    return;
...
{
    // FJAR does not support SegWit (NO_SEGWIT flag). CPFP functionality itself works,
    // but this specific test constructs SegWit transactions for validation.
    return;
    ...
    // The mempool minimum feerate is 5sat/vB, but this package just pays 800 satooshis total.
    ...
    // Now that the child's fees have "increased" by 1 FJAR, the cheap package should succeed.
    {
    ...
    // Parent pays 1 FJAR and child pays none. The parent should be accepted without the child.
    {
        BOOST_CHECK_EQUAL(m_node.mempool->size(), expected_pool_size);
