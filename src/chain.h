...
#ifndef FJARCODE_CHAIN_H
#define FJARCODE_CHAIN_H
 ...
 *
 * Ref: https://github.com/fjarcode/fjarcode/pull/1026
 */
...
enum BlockStatus : uint32_t {
    ...
    /**
     * FJAR/BCHN: Block has been parked (temporarily rejected).
     * Parked blocks are similar to invalid blocks but can be unparked
     * (reconsidered) via RPC. Used for automatic finalization (rolling
     * checkpoints) to reject deep reorgs.
     */
    BLOCK_PARKED             =   512,

    /**
     * FJAR/BCHN: Descends from a parked block.
     */
...
public:
    ...
    //! (memory only) ABLA: Block size in bytes (set during ConnectBlock)
    uint64_t nBlockSize{0};
    //! (memory only) ABLA: Control block size state after this block
    uint64_t nABLAControlBlockSize{0};
    //! (memory only) ABLA: Elastic buffer size state after this block
    uint64_t nABLAElasticBufferSize{0};
    ...
    //! FJAR/BCHN: @returns true if the block is parked (temporarily rejected).
    //! Parked blocks are used for automatic finalization to prevent deep reorgs.
    bool IsParked() const EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
    {
        AssertLockHeld(::cs_main);
        return nStatus & BLOCK_PARKED_MASK;
    }
