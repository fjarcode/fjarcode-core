...
bool MutableTransactionSignatureCreator::CreateSig(const SigningProvider& provider, std::vector<unsigned char>& vchSig, const CKeyID& address, const CScript& scriptCode, SigVersion sigversion) const
{
    ...
    // Signing without known amount does not work in witness or BCH_FORKID scripts (BIP143 sighash).
    if ((sigversion == SigVersion::WITNESS_V0 || sigversion == SigVersion::BCH_FORKID) && !MoneyRange(amount)) return false;
...
{
    return ProduceSignature(provider, creator, fromPubKey, sigdata, false);
}

bool ProduceSignature(const SigningProvider& provider, const BaseSignatureCreator& creator, const CScript& fromPubKey, SignatureData& sigdata, bool fjarcode_witness_via_scriptsig)
{
    ...
    if (solved && whichType == TxoutType::WITNESS_V0_KEYHASH)
    {
        ...
        TxoutType subType;
        SigVersion sigver = fjarcode_witness_via_scriptsig ? SigVersion::BCH_FORKID : SigVersion::WITNESS_V0;
        solved = solved && SignStep(provider, creator, witnessscript, result, subType, sigver, sigdata);
        if (!fjarcode_witness_via_scriptsig) {
            // Pre-fork: put in witness
            sigdata.scriptWitness.stack = result;
            sigdata.witness = true;
            result.clear();
        }
        // Post-fork: leave result for scriptSig
    }
    else if (solved && whichType == TxoutType::WITNESS_V0_SCRIPTHASH)
    {
        ...
        SigVersion sigver = fjarcode_witness_via_scriptsig ? SigVersion::BCH_FORKID : SigVersion::WITNESS_V0;
        solved = solved && SignStep(provider, creator, witnessscript, result, subType, sigver, sigdata) && subType != TxoutType::SCRIPTHASH && subType != TxoutType::WITNESS_V0_SCRIPTHASH && subType != TxoutType::WITNESS_V0_KEYHASH;
        ...
        // Post-fork: leave result for scriptSig (Schnorr signature)
    ...
    // Test solution — use FJAR flags post-fork (includes opcode enablement)
    unsigned int verify_flags = fjarcode_witness_via_scriptsig ? FJARCODE_SCRIPT_VERIFY_FLAGS : STANDARD_SCRIPT_VERIFY_FLAGS;
...
public:
    ...
    bool CheckBCHSchnorrSignature(const std::vector<unsigned char>& sig, const std::vector<unsigned char>& vchPubKey, const CScript& scriptCode, SigVersion sigversion) const override { return sig.size() != 0; }
...
bool SignTransaction(CMutableTransaction& mtx, const SigningProvider* keystore, const std::map<COutPoint, Coin>& coins, int nHashType, std::map<int, bilingual_str>& input_errors)
{
    ...
    for (unsigned int i = 0; i < mtx.vin.size(); ++i) {
        ...
        bool fjarcode_witness_via_scriptsig = (nHashType & SIGHASH_FORKID) != 0;
        // Only sign SIGHASH_SINGLE if there's a corresponding output:
        if (!fHashSingle || (i < mtx.vout.size())) {
            ProduceSignature(*keystore, MutableTransactionSignatureCreator(mtx, i, amount, &txdata, nHashType), prevPubKey, sigdata, fjarcode_witness_via_scriptsig);
        }

        UpdateInput(txin, sigdata);

        // amount must be specified for valid segwit signature (pre-fork only)
        if (!fjarcode_witness_via_scriptsig && amount == MAX_MONEY && !txin.scriptWitness.IsNull()) {
        ...
        if (fjarcode_witness_via_scriptsig) {
            // Post-fork: use FJAR flags (requires FORKID, no WITNESS/TAPROOT)
            verify_flags &= ~(SCRIPT_VERIFY_WITNESS | SCRIPT_VERIFY_TAPROOT);
            verify_flags |= SCRIPT_VERIFY_NO_SEGWIT | SCRIPT_VERIFY_SIGHASH_FORKID;
        }
