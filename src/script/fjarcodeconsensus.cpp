...
// This is retained for historical block validation and pre-fork transaction support.
// After FJAR fork, new transactions have no witness data.
...
{
    return (flags & ~(fjarcodeconsensus_SCRIPT_FLAGS_VERIFY_ALL)) == 0;
}
                                    ...
                                    unsigned int nIn, unsigned int flags, fjarcodeconsensus_error* err)
{
    if (!verify_flags(flags)) {
        return set_error(err, fjarcodeconsensus_ERR_INVALID_FLAGS);
    }

    if (flags & fjarcodeconsensus_SCRIPT_FLAGS_VERIFY_TAPROOT && spentOutputs == nullptr) {
    ...
    try {
        ...
        if (nIn >= tx.vin.size())
            return set_error(err, fjarcodeconsensus_ERR_TX_INDEX);
        if (GetSerializeSize(TX_WITH_WITNESS(tx)) != txToLen)
            return set_error(err, fjarcodeconsensus_ERR_TX_SIZE_MISMATCH);
        ...
        if (spentOutputs != nullptr && flags & fjarcodeconsensus_SCRIPT_FLAGS_VERIFY_TAPROOT) {
        ...
        return set_error(err, fjarcodeconsensus_ERR_TX_DESERIALIZE); // Error deserializing
    }
}

int fjarcodeconsensus_verify_script_with_spent_outputs(const unsigned char *scriptPubKey, unsigned int scriptPubKeyLen, int64_t amount,
                                    const unsigned char *txTo        , unsigned int txToLen,
                                    const UTXO *spentOutputs, unsigned int spentOutputsLen,
                                    unsigned int nIn, unsigned int flags, fjarcodeconsensus_error* err)
{
...
int fjarcodeconsensus_verify_script_with_amount(const unsigned char *scriptPubKey, unsigned int scriptPubKeyLen, int64_t amount,
                                    const unsigned char *txTo        , unsigned int txToLen,
                                    unsigned int nIn, unsigned int flags, fjarcodeconsensus_error* err)
{
    CAmount am(amount);
...
int fjarcodeconsensus_verify_script(const unsigned char *scriptPubKey, unsigned int scriptPubKeyLen,
                                   const unsigned char *txTo        , unsigned int txToLen,
                                   unsigned int nIn, unsigned int flags, fjarcodeconsensus_error* err)
{
