...
#ifndef FJARCODE_TXORPHANAGE_H
#define FJARCODE_TXORPHANAGE_H
...
protected:
    ...
    std::map<NodeId, unsigned int> m_peer_orphan_count GUARDED_BY(m_mutex);

    /** FJAR: Maximum orphans per peer (prevents single peer DoS).
     *  Set to 1/3 of DEFAULT_MAX_ORPHAN_TRANSACTIONS to ensure no single peer
     *  can fill more than 1/3 of the global orphan pool. */
    static constexpr unsigned int MAX_ORPHANS_PER_PEER{33};

    /** Erase an orphan by txid */
    int EraseTxNoLock(const Txid& txid) EXCLUSIVE_LOCKS_REQUIRED(m_mutex);
};

#endif // FJARCODE_TXORPHANAGE_H
