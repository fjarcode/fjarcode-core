...
#ifndef FJARCODE_VALIDATION_H
#define FJARCODE_VALIDATION_H
...
protected:
    ...
    //! FJAR/BCHN: Height of the most recently finalized block (rolling checkpoint).
    //! Blocks at or below this height cannot be reorged away.
    //! -1 means no finalization has occurred yet.
    int m_finalized_height GUARDED_BY(::cs_main) {-1};
...
public:
    ...
    // =========================================================================
    // FJAR/BCHN Automatic Finalization (Rolling Checkpoints)
    // =========================================================================

    /** FJAR/BCHN: Park a block (temporarily reject it).
     *  Parked blocks are like invalid blocks but can be unparked.
     *  Used to implement automatic finalization / rolling checkpoints.
     */
    bool ParkBlock(BlockValidationState& state, CBlockIndex* pindex)
        EXCLUSIVE_LOCKS_REQUIRED(!m_chainstate_mutex)
        LOCKS_EXCLUDED(::cs_main);

    /** FJAR/BCHN: Unpark a block (remove parked status).
     *  Similar to ResetBlockFailureFlags but for parked blocks.
     */
    void UnparkBlock(CBlockIndex* pindex) EXCLUSIVE_LOCKS_REQUIRED(cs_main);

    /** FJAR/BCHN: Unpark all parked blocks. */
    void UnparkBlockAndDescendants(CBlockIndex* pindex) EXCLUSIVE_LOCKS_REQUIRED(cs_main);

    /** FJAR/BCHN: Manually finalize a block.
     *  This explicitly marks a block and all ancestors as final.
     *  Any competing chain that doesn't contain this block will be parked.
     *  Returns false if the block is not in the active chain.
     */
    bool FinalizeBlock(BlockValidationState& state, CBlockIndex* pindex)
        EXCLUSIVE_LOCKS_REQUIRED(!m_chainstate_mutex)
        LOCKS_EXCLUDED(::cs_main);

    /** FJAR/BCHN: Get the current finalized block height.
     *  Returns the height of the most recently finalized block,
     *  or -1 if no block has been finalized yet.
     */
    int GetFinalizedHeight() const EXCLUSIVE_LOCKS_REQUIRED(cs_main)
    {
        AssertLockHeld(cs_main);
        return m_finalized_height;
    }

    /** FJAR/BCHN: Check if a reorg would cross the finalization boundary.
     *  Returns true if the reorg should be rejected.
     */
    bool WouldCrossFinalization(const CBlockIndex* pindexFork) const
        EXCLUSIVE_LOCKS_REQUIRED(cs_main);

    /** FJAR/BCHN: Update automatic finalization after connecting a new block.
     *  Called from ConnectTip to finalize blocks that are deep enough.
     */
    void UpdateAutomaticFinalization(const CBlockIndex* pindexNew)
        EXCLUSIVE_LOCKS_REQUIRED(cs_main);
