                                                  ...
                                                  CTxMemPool::setEntries& all_conflicts)
{
    ...
    // This avoids overestimation when multiple conflicts share descendants
    for (CTxMemPool::txiter it : iters_conflicting) {
        pool.CalculateDescendants(it, all_conflicts);
    }

    // Rule #5: don't consider replacing more than MAX_REPLACEMENT_CANDIDATES
    // entries from the mempool. Now using actual set size instead of estimated count.
    if (all_conflicts.size() > MAX_REPLACEMENT_CANDIDATES) {
        return strprintf("rejecting replacement %s; too many potential replacements (%u > %d)\n",
                         txid.ToString(),
                         all_conflicts.size(),
