...
// backward compatibility with pre-fork PSBTs and spending historical UTXOs.
...
bool PSBTInputSignedAndVerified(const PartiallySignedTransaction psbt, unsigned int input_index, const PrecomputedTransactionData* txdata)
{
    ...
    // FJAR: Use FJARCODE_SCRIPT_VERIFY_FLAGS which includes SIGHASH_FORKID
    // and excludes WITNESS/TAPROOT (not supported on FJAR).
    if (txdata) {
        return VerifyScript(input.final_script_sig, utxo.scriptPubKey, &input.final_script_witness, FJARCODE_SCRIPT_VERIFY_FLAGS, MutableTransactionSignatureChecker{&(*psbt.tx), input_index, utxo.nValue, *txdata, MissingDataBehavior::FAIL});
    } else {
        return VerifyScript(input.final_script_sig, utxo.scriptPubKey, &input.final_script_witness, FJARCODE_SCRIPT_VERIFY_FLAGS, MutableTransactionSignatureChecker{&(*psbt.tx), input_index, utxo.nValue, MissingDataBehavior::FAIL});
    }
}
...
bool FinalizePSBT(PartiallySignedTransaction& psbtx, int sighash_type)
{
...
bool FinalizeAndExtractPSBT(PartiallySignedTransaction& psbtx, CMutableTransaction& result, int sighash_type)
{
    // It's not safe to extract a PSBT that isn't finalized, and there's no easy way to check
    //   whether a PSBT is finalized without finalizing it, so we just do this.
    if (!FinalizePSBT(psbtx, sighash_type)) {
        return false;
    }
