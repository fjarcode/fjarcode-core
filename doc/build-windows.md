...
Below are some notes on how to build FJARCODE Core for Windows.

The options known to work for building FJARCODE Core on Windows are:
...
Note that for WSL the FJARCODE Core source path MUST be somewhere in the default mount file system, for
example /usr/src/fjarcode-core, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\fjarcode`, for example:

    make install DESTDIR=/mnt/c/workspace/fjarcode
