# Support for Output Descriptors in FJARCODE

Since Bitcoin Core v0.17, there is support for Output Descriptors. This is a
simple language which can be used to describe collections of output scripts.

**FJAR Note:** After the FJAR fork activation, SegWit is disabled. The following
descriptor functions are **not available** for new addresses/transactions:
- `wpkh()` - P2WPKH (native SegWit)
- `wsh()` - P2WSH (native SegWit script)
- `tr()` - P2TR (Taproot)
- Any descriptors containing witness scripts

Only `pk()`, `pkh()`, `sh()`, `multi()`, `sortedmulti()`, `combo()`, `addr()`,
and `raw()` are supported after the fork. The `combo()` function only generates
P2PK and P2PKH outputs (not P2WPKH or P2SH-P2WPKH) after the fork.
Supporting RPCs are:
