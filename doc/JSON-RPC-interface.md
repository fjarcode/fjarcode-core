...
```sh
# Get block count from the / endpoint when rpcuser=alice and rpcport=30442
$ curl --user alice --data-binary '{"jsonrpc": "1.0", "id": "0", "method": "getblockcount", "params": []}' -H 'content-type: text/plain;' localhost:30442/

# Get balance from the /wallet/walletname endpoint when rpcuser=alice, rpcport=30442 and rpcwallet=desc-wallet
$ curl --user alice --data-binary '{"jsonrpc": "1.0", "id": "0", "method": "getbalance", "params": []}' -H 'content-type: text/plain;' localhost:30442/wallet/desc-wallet
