...
fn lint_includes_build_config() -> LintResult {
    let config_path = "./src/config/fjarcode-config.h.in";
    let include_directive = "#include <config/fjarcode-config.h>";
    ...
    let print_affected_files = |mode: bool| {
        ...
        let defines_files = check_output(
            git()
                ...
                .args([
                    // These are exceptions which don't use fjarcode-config.h, rather the Makefile.am adds
    ...
    if missing {
        return Err(format!(
            r#"
^^^
One or more files use a symbol declared in the fjarcode-config.h header. However, they are not
...
even though fjarcode-config.h indicates that a faster feature is available and should be used.
...
git grep --perl-regexp '{}' -- file_name
    ...
    if redundant {
        return Err(r#"
^^^
None of the files use a symbol declared in the fjarcode-config.h header. However, they are including
