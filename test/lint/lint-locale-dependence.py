...
#
# Be aware that fjarcoded and fjarcode-qt differ in terms of localization: Qt
# opts in to POSIX localization by running setlocale(LC_ALL, "") on startup,
# whereas no such call is made in fjarcoded.
#
...
# C locale.
#
# In contrast, fjarcoded does not opt in to localization -- no call to
# setlocale(LC_ALL, "") is made and the environment variables LC_* are
# thus ignored.
#
# This results in situations where fjarcoded is guaranteed to be running
# with the classic locale ("C") whereas the locale of fjarcode-qt will vary
# depending on the user's environment variables.
#
# An example: Assuming the environment variable LC_ALL=de_DE then the
# call std::to_string(1.23) will return "1.230000" in fjarcoded but
# "1,230000" in fjarcode-qt.
#
