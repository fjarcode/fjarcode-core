...
BASE_SCRIPTS = [
    ...
    'wallet_migration.py',
    # 'p2p_ibd_txrelay.py',
    # FJAR-specific functional tests
    'feature_fjarcode_consensus.py',
    'feature_fjarcode_abla.py',
    'feature_fjarcode_tx_lifecycle.py --legacy-wallet',
    'feature_fjarcode_rbf_disabled.py --legacy-wallet',
    'feature_fjarcode_finalization.py',
    'feature_fjarcode_wallet_import.py --legacy-wallet',
    'feature_fjarcode_wallet_segwit.py --legacy-wallet',
    'feature_fjarcode_consensus_edge.py --legacy-wallet',
    'p2p_dsproof.py --legacy-wallet',
    'p2p_fjarcode_reorg.py',
    'p2p_fjarcode_adversarial.py',
    # Don't append tests at the end to avoid merge conflicts
    # Put them in a random line within the section that fits their approximate run-time
]

# SegWit/Taproot tests - FJAR removed SegWit and Taproot
# RBF tests - FJAR disabled Replace-By-Fee
FJARCODE_EXCLUDED_SEGWIT_RBF = [
    'feature_taproot.py',
    'p2p_segwit.py',
...
def main():
    ...
    enable_fjarcoded = config["components"].getboolean("ENABLE_FJARCODED")

    if not enable_fjarcoded:
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control, skipunit=False):
    ...
    # Warn if fjarcoded is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "fjarcoded"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a fjarcoded process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
    ...
    # a hard link or a copy on any platform. See https://github.com/fjarcode/fjarcode/pull/27561.
...
class RPCCoverage():
    ...
    commands per `fjarcode-cli help` (`rpc_interface.txt`).
