...
class PeerTxRelayer(P2PTxInvStore):
    ...
    def wait_for_parent_requests(self, txids):
        """Wait for requests for missing parents by txid with witness data (MSG_TX or
...
class OrphanHandlingTest(BitcoinTestFramework):
    ...
    def relay_transaction(self, peer, tx):
        ...
        peer.send_and_ping(msg_inv([CInv(t=MSG_TX, h=wtxid)]))
    ...
    def test_arrival_timing_orphan(self):
        ...
        parent_inv = CInv(t=MSG_TX, h=int(tx_parent_arrives["tx"].getwtxid(), 16))
    ...
    def test_orphans_overlapping_parents(self):
        ...
        peer_txrequest.send_and_ping(msg_inv([CInv(t=MSG_TX, h=int(inflight_parent_AB["tx"].getwtxid(), 16))]))
        self.nodes[0].bumpmocktime(NONPREF_PEER_TX_DELAY)
