...
class KeyPoolTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        if self.options.descriptors:
            # FJAR: Only import pkh descriptors (no SegWit support)
        ...
        addr = set()
        addr.add(nodes[0].getnewaddress(address_type="legacy"))
        addr.add(nodes[0].getnewaddress(address_type="legacy"))
        addr.add(nodes[0].getnewaddress(address_type="legacy"))
        addr.add(nodes[0].getnewaddress(address_type="legacy"))
        addr.add(nodes[0].getnewaddress(address_type="legacy"))
        ...
        res = w2.walletcreatefundedpsbt(inputs=[], outputs=[{destination: 0.00010000}], subtractFeeFromOutputs=[0], feeRate=0.0004)
        assert_equal("psbt" in res, True)
