...
class ListTransactionsTest(BitcoinTestFramework):
    ...
    def run_rbf_opt_in_test(self):
        ...
        self.log.info("Test txs with low nSequence (FJAR: no RBF, always bip125-replaceable=no)")
        # Tx3 has low nSequence (would be opt-in RBF in Bitcoin Core, but not in FJAR)
        ...
        # FJAR: No RBF support, bip125-replaceable is always "no"
        assert_array_result(self.nodes[0].listtransactions(), {"txid": txid_3}, {"bip125-replaceable": "no"})
        self.sync_mempools()
        assert_array_result(self.nodes[1].listtransactions(), {"txid": txid_3}, {"bip125-replaceable": "no"})

        # Tx4 will chain off tx3.
        ...
        # FJAR: No RBF, so even children of opt-in parents are "no"
        ...
        # FJAR: Skip RBF replacement test (tx3_b) since FJAR has no RBF

        self.log.info("Test bip125-replaceable status with gettransaction RPC")
        for n in self.nodes[0:2]:
            assert_equal(n.gettransaction(txid_1)["bip125-replaceable"], "no")
            assert_equal(n.gettransaction(txid_2)["bip125-replaceable"], "no")
            assert_equal(n.gettransaction(txid_3)["bip125-replaceable"], "no")
        ...
        for n in self.nodes[0:2]:
            ...
            assert_equal(txs[txid_3], "no")
            assert_equal(txs[txid_4], "no")

        self.log.info("Test mined transactions are no longer bip125-replaceable")
        self.generate(self.nodes[0], 1)
        assert_equal(self.nodes[0].gettransaction(txid_3)["bip125-replaceable"], "no")
    ...
    def run_externally_generated_address_test(self):
        ...
        addr2 = self.nodes[0].getnewaddress("pizza2", 'legacy')
        addr3 = self.nodes[0].getnewaddress("pizza3", 'legacy')
    ...
    def run_coinjoin_test(self):
        ...
        # But it is not, see for example https://github.com/fjarcode/fjarcode/issues/14136:
