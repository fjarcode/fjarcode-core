...
class MempoolAcceptanceTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.mempool_size = 0
        # Also check feerate. 1FJARCODE/kvB fails
        assert_raises_rpc_error(-8, "Fee rates larger than or equal to 1FJARCODE/kvB are not accepted", lambda: self.check_mempool_result(
        ...
        utxo_to_spend = self.wallet.get_utxo(txid=txid_in_block)  # use 0.3 FJAR UTXO
        ...
        tx._fjarcode_input_amounts = [int(0.3 * COIN)]
        ...
        tx = tx_from_hex(raw_tx_0)
        tx._fjarcode_input_amounts = [int(49 * COIN)]
        ...
        tx._fjarcode_input_amounts = [int((Decimal('0.3') - 2 * fee) * COIN), int((Decimal('0.3') - fee) * COIN)]
        self.wallet.sign_tx(tx)
        ...
        raw_tx_reference_amounts = list(tx._fjarcode_input_amounts)
