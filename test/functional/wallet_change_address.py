...
class WalletChangeAddressTest(BitcoinTestFramework):
    ...
    def set_test_params(self):
        ...
        # FJAR: Use lower discardfee for P2PKH (larger inputs increase cost_of_change)
        self.extra_args = [
            [],
            ["-discardfee=0.1"],
    ...
    def run_test(self):
        ...
        # Send 5 + 2.5 coin to each address (larger amounts for P2PKH to avoid
        # change being discarded due to high -discardfee with larger P2PKH inputs)
        [self.nodes[0].sendtoaddress(addr, 5.0) for addr in addrs]
        [self.nodes[0].sendtoaddress(addr, 2.5) for addr in addrs]
