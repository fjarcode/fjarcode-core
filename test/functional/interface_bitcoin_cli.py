...
# The block reward of coinbaseoutput.nValue (50) FJAR/block matures after
# COINBASE_MATURITY (100) blocks. Therefore, after mining 101 blocks we expect
# node 0 to have a balance of (BLOCKS - COINBASE_MATURITY) * 50 FJAR/block.
...
class TestBitcoinCli(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Compare responses from getblockchaininfo RPC and `fjarcode-cli getblockchaininfo`")
        ...
        self.log.info("Test -getinfo and fjarcode-cli return all proxies")
        ...
        if self.is_specified_wallet_compiled():
            self.log.info("Test -getinfo and fjarcode-cli getwalletinfo return expected wallet info")
            self.restart_node(0, extra_args=["-addresstype=legacy", "-changetype=legacy"])
            ...
            assert_equal(Decimal(cli_get_info['Transaction fee rate (-paytxfee) (FJAR/kvB)']), wallet_info['paytxfee'])
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (FJAR/kvB)']), network_info['relayfee'])
            ...
            # Mine a block to confirm; adds a block reward (50 FJAR) to the default wallet.
            self.generate(self.nodes[0], 1)

            amounts = [w1.getbalance(), Decimal(9), Decimal(31)]
            ...
            # Test fjarcode-cli -generate with -rpcwallet in multiwallet mode.
