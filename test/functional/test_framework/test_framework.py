...
from .address import key_to_p2pkh
...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave fjarcodeds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop fjarcodeds after the test execution")
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "fjarcoded": ("fjarcoded", "FJARCODED"),
            "fjarcode-cli": ("fjarcodecli", "FJARCODECLI"),
            "fjarcode-util": ("fjarcodeutil", "FJARCODEUTIL"),
            "fjarcode-wallet": ("fjarcodewallet", "FJARCODEWALLET"),
        }
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: fjarcodeds were not stopped and may still be running")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'fjarcoded', self.options.fjarcoded) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'fjarcode-cli', self.options.fjarcodecli) for v in versions]
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the fjarcoded
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as fjarcoded's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    fjarcoded=self.options.fjarcoded,
                    fjarcode_cli=self.options.fjarcodecli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
            ...
            _mw_key = ECKey()
            _mw_key.set((1).to_bytes(32, 'big'), True)
            gen_addresses = [k.address for k in TestNode.PRIV_KEYS][:3] + [key_to_p2pkh(_mw_key.get_pubkey().get_bytes())]
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in fjarcode.conf
    ...
    def skip_if_no_fjarcoded_tracepoints(self):
        """Skip the running test if fjarcoded has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("fjarcoded has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_fjarcoded_zmq(self):
        """Skip the running test if fjarcoded has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("fjarcoded has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if fjarcode-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("fjarcode-wallet has not been compiled")

    def skip_if_no_fjarcode_util(self):
        """Skip the running test if fjarcode-util has not been compiled."""
        if not self.is_fjarcode_util_compiled():
            raise SkipTest("fjarcode-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if fjarcode-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("fjarcode-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether fjarcode-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether fjarcode-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_fjarcode_util_compiled(self):
        """Checks whether fjarcode-util was compiled."""
        return self.config["components"].getboolean("ENABLE_FJARCODE_UTIL")
