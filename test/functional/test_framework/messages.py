...
COIN = 100000000  # 1 FJAR in satooshis
MAX_MONEY = 21000000 * COIN

MAX_BIP125_RBF_SEQUENCE = 0xfffffffd  # Sequence number that is rbf-opt-in (BIP 125) and csv-opt-out (BIP 68)
SEQUENCE_FINAL = 0xffffffff  # Sequence number that disables nLockTime if set for every input of a tx

MAX_PROTOCOL_MESSAGE_LENGTH = 33 * 1000 * 1000  # Maximum length of incoming protocol messages (FJAR: 33MB for 32MB blocks)
...
# Matches BCH's relay policy for OP_RETURN.
...
MAGIC_BYTES = {
    ...
    "regtest": b"\xb2\xc2\xfa\xbf",   # regtest (FJAR-unique)
...
# Objects that map to fjarcoded objects, which can be serialized/deserialized


class CAddress:
    __slots__ = ("net", "ip", "nServices", "port", "time")

    # see https://github.com/fjarcode/bips/blob/master/bip-0155.mediawiki
    NET_IPV4 = 1
    NET_IPV6 = 2
    NET_TORV3 = 4
    NET_I2P = 5
...
class CInv:
    ...
    typemap = {
        ...
        MSG_TX: "TX",
        MSG_BLOCK: "Block",
        MSG_TX | (1 << 30): "WitnessTx",
        MSG_BLOCK | (1 << 30): "WitnessBlock",
    ...
    def __repr__(self):
        return "CInv(type=%s hash=%064x)" \
            % (self.typemap.get(self.type, "Unknown(%d)" % self.type), self.hash)
...
class CTransaction:
    ...
    def __init__(self, tx=None):
        if tx is None:
            ...
            self.vout = []
            self.wit = CTxWitness()
            self._fjarcode_input_amounts = None
            self.nLockTime = 0
            self.sha256 = None
            self.hash = None
        else:
            ...
            self._fjarcode_input_amounts = getattr(tx, '_fjarcode_input_amounts', None)
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in fjarcoded
            if (flags != 0):
                self.vin = deser_vector(f, CTxIn)
...
class msg_version:
    ...
    def deserialize(self, f):
        ...
        # But, unconditionally check it to match behaviour in fjarcoded
...
class msg_headers:
    ...
    def deserialize(self, f):
        # comment in fjarcoded indicates these should be deserialized as blocks
