...
class MiniWalletMode(Enum):
    ...
    it uses a deterministic P2PKH address whose coins are spent with a legacy
    ECDSA signature. (FJAR: changed from P2TR/SegWit to P2PKH since SegWit is
    disabled after the fork.)

                    |      output       |           |  tx is   | can modify |  needs
         mode       |    description    |  address  | standard | scriptSig  | signing
    ----------------+-------------------+-----------+----------+------------+----------
    ADDRESS_OP_TRUE | pay-to-pubkey-hash|  P2PKH    |   yes    |    no      |   yes
...
class MiniWallet:
    def __init__(self, test_node, *, mode=MiniWalletMode.ADDRESS_OP_TRUE):
        ...
        if mode == MiniWalletMode.RAW_OP_TRUE:
            ...
            self._priv_key.set((1).to_bytes(32, 'big'), True)
            pub_key = self._priv_key.get_pubkey()
            self._pub_key_bytes = pub_key.get_bytes()
            self._scriptPubKey = key_to_p2pkh_script(self._pub_key_bytes)
            self._address = key_to_p2pkh(self._pub_key_bytes)
    ...
    def sign_tx(self, tx, fixed_length=True, input_amounts=None):
        if input_amounts is None:
            input_amounts = getattr(tx, '_fjarcode_input_amounts', None)
        if self._mode == MiniWalletMode.RAW_P2PK:
            ...
            SIGHASH_FORKID = 0x40
            sighash_type = SIGHASH_ALL | SIGHASH_FORKID  # 0x41
            for i in range(len(tx.vin)):
                amount = input_amounts[i] if input_amounts else 0
                tx.vin[i].scriptSig = b''
                while not len(tx.vin[i].scriptSig) == 107:
                    tx.vin[i].scriptSig = CScript([self._pub_key_bytes])
                    sighash = SegwitV0SignatureHash(self._scriptPubKey, tx, i, sighash_type, amount)
                    der_sig = self._priv_key.sign_ecdsa(sighash)
                    tx.vin[i].scriptSig = bytes(CScript([der_sig + bytes([sighash_type])])) + tx.vin[i].scriptSig
                    tx.rehash()
                    if not fixed_length:
                        break
        else:
    ...
    def get_descriptor(self):
        if self._mode == MiniWalletMode.ADDRESS_OP_TRUE:
            return descsum_create(f'pkh({self._pub_key_bytes.hex()})')
    ...
    def send_to(self, *, from_node, scriptPubKey, amount, fee=1000):
        ...
        """
        utxo = self.get_utxo()
        input_value = int(COIN * utxo['value'])
        assert_greater_than_or_equal(input_value, amount + fee)
        tx = CTransaction()
        tx.vin = [CTxIn(COutPoint(int(utxo['txid'], 16), utxo['vout']), nSequence=0)]
        tx.vout = [CTxOut(input_value - amount - fee, bytearray(self._scriptPubKey))]  # change
        tx.vout.append(CTxOut(amount, scriptPubKey))  # recipient
        tx.nVersion = 2
        self.sign_tx(tx, input_amounts=[input_value])
    ...
    def create_self_transfer_multi(
        ...
        *,
        ...
        input_amounts = [int(COIN * utxo['value']) for utxo in utxos_to_spend]
        tx._fjarcode_input_amounts = input_amounts  # Store for re-signing after modification
        self.sign_tx(tx, input_amounts=input_amounts)
