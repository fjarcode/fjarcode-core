...
def assert_fee_amount(fee, tx_size, feerate_BC2_kvB):
    ...
    if fee < target_fee:
        raise AssertionError("Fee of %s FJAR too low! (Should be %s FJAR)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    high_fee = get_fee(tx_size + 2, feerate_BC2_kvB)
    if fee > high_fee:
        raise AssertionError("Fee of %s FJAR too high! (Should be %s FJAR)" % (str(fee), str(target_fee)))
...
def check_json_precision():
    """Make sure json library being used does not lose precision converting FJAR values"""
...
def get_fee(tx_size, feerate_bc2_kvb):
    """Calculate the fee in FJAR given a feerate is FJAR/kvB. Reflects CFeeRate::GetFee"""
    ...
    return target_fee_sat / Decimal(1e8) # Return result in  FJAR
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "fjarcode.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def get_temp_default_datadir(temp_dir: pathlib.Path) -> tuple[dict, pathlib.Path]:
    ...
    if platform.system() == "Windows":
        ...
        datadir = temp_dir / "FJARCODE"
    else:
        env = dict(HOME=str(temp_dir))
        if platform.system() == "Darwin":
            datadir = temp_dir / "Library/Application Support/FJARCODE"
        else:
            datadir = temp_dir / ".fjarcode"
    return env, datadir
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "fjarcode.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "fjarcode.conf")):
        with open(os.path.join(datadir, "fjarcode.conf"), 'r', encoding='utf8') as f:
            for line in f:
