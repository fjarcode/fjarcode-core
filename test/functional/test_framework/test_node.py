...
FJARCODED_PROC_WAIT_TIMEOUT = 60
...
class TestNode():
    ...
    def __init__(self, i, datadir_path, *, chain, rpchost, timewait, timeout_factor, fjarcoded, fjarcode_cli, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None, descriptors=False, v2transport=False):
        """
        Kwargs:
        ...
        # This means that starting a fjarcoded using the temp dir to debug a failed test won't
        # spam debug.log.
        self.args = [
            self.binary,
        ...
        self.cli = TestNodeCLI(fjarcode_cli, self.datadir_path)
    ...
    def wait_for_rpc_connection(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcpassword; fjarcoded is still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error("Unable to connect to fjarcoded after {}s".format(self.rpc_timeout))
    ...
    def wait_until_stopped(self, *, timeout=FJARCODED_PROC_WAIT_TIMEOUT, expect_error=False, **kwargs):
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        Will throw if an expected_msg is provided and it does not match fjarcoded's stdout."""
...
class TestNodeCLI():
    """Interface to fjarcode-cli for an individual node"""
    def __init__(self, binary, datadir):
        self.options = []
        self.binary = binary
        self.datadir = datadir
        self.input = None
        self.log = logging.getLogger('TestFramework.fjarcodecli')

    def __call__(self, *options, input=None):
        # TestNodeCLI is callable with fjarcode-cli command-line options
    ...
    def send_cli(self, clicommand=None, *args, **kwargs):
        """Run fjarcode-cli command. Deserializes returned string as python object."""
        ...
        self.log.debug("Running fjarcode-cli {}".format(p_args[2:]))
