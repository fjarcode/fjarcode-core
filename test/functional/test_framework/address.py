...
# FJAR CashAddr encoding
CASHADDR_CHARSET = "qpzry9x8gf2tvdw0s3jn54khce6mua7l"
CASHADDR_PREFIX_MAINNET = "fjarcode"
CASHADDR_PREFIX_TESTNET = "bchiitest"
CASHADDR_PREFIX_REGTEST = "fjarcode"  # regtest uses same prefix
...
def _cashaddr_expand_prefix(prefix):
    return [ord(c) & 0x1f for c in prefix] + [0]

def _cashaddr_pack_addr_data(hash_bytes, addr_type=0):
    """Pack hash bytes into 5-bit CashAddr data with version byte."""
    version_byte = (addr_type << 3) | 0  # type + size=0 for 20-byte hash
    acc = version_byte
    bits = 8
    result = []
    for byte in hash_bytes:
        ...
        while bits >= 5:
            bits -= 5
            result.append((acc >> bits) & 0x1f)
    if bits > 0:
        result.append((acc << (5 - bits)) & 0x1f)
    return result

def cashaddr_encode(prefix, addr_type, hash_bytes):
    """Encode a CashAddr address. addr_type: 0=P2PKH, 1=P2SH."""
    payload = _cashaddr_pack_addr_data(hash_bytes, addr_type)
    expanded = _cashaddr_expand_prefix(prefix)
    mod = _cashaddr_polymod(expanded + payload + [0]*8)
    checksum = [(mod >> (5 * (7 - i))) & 0x1f for i in range(8)]
    return prefix + ':' + ''.join(CASHADDR_CHARSET[d] for d in payload + checksum)

def cashaddr_decode(address):
    """Decode a CashAddr address. Returns (addr_type, hash_bytes) or (None, None) on failure."""
    if ':' not in address:
        return None, None
    prefix, payload_str = address.split(':', 1)
    try:
        data = [CASHADDR_CHARSET.index(c) for c in payload_str.lower()]
    except ValueError:
        return None, None
    expanded = _cashaddr_expand_prefix(prefix.lower())
    if _cashaddr_polymod(expanded + data) != 0:
        return None, None
    # Remove the 8 checksum values
    payload = data[:-8]
    ...
    version_byte = result[0]
    addr_type = (version_byte >> 3) & 0x1f
    hash_bytes = bytes(result[1:])
    return addr_type, hash_bytes

def keyhash_to_p2pkh(hash, main=False):
    assert len(hash) == 20
    prefix = CASHADDR_PREFIX_MAINNET if main else CASHADDR_PREFIX_REGTEST
    return cashaddr_encode(prefix, 0, hash)

def scripthash_to_p2sh(hash, main=False):
    assert len(hash) == 20
    prefix = CASHADDR_PREFIX_MAINNET if main else CASHADDR_PREFIX_REGTEST
    return cashaddr_encode(prefix, 1, hash)
...
def address_to_scriptpubkey(address):
    ...
    addr_type, hash_bytes = cashaddr_decode(address)
    if hash_bytes is not None:
        if addr_type == 0:  # P2PKH
            return keyhash_to_p2pkh_script(hash_bytes)
        elif addr_type == 1:  # P2SH
            return scripthash_to_p2sh_script(hash_bytes)
...
# FJAR: Unspendable P2PKH CashAddr address (hash160 = all zeros, no one has the private key)
ADDRESS_FJARCODE_UNSPENDABLE = cashaddr_encode(CASHADDR_PREFIX_REGTEST, 0, b'\x00' * 20)
