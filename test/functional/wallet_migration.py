...
class WalletMigrationTest(BitcoinTestFramework):
    ...
    def test_basic(self):
        ...
        # FJAR: The wallet should create the following descriptors:
        # * BIP32 combo descriptors in the form of "0h/0h/*h" and "0h/1h/*h" (2 descriptors)
        # * BIP44 pkh descriptors in the form of "44h/1h/0h/0/*" and "44h/1h/0h/1/*" (2 descriptors)
        # * A combo(PK) descriptor for the wallet master key (1 descriptor)
        # * sh(wpkh()) descriptors for legacy SegWit scripts in mapScripts (5 descriptors)
        # * wpkh() descriptors for legacy SegWit scripts in mapScripts (5 descriptors)
        # So, should have a total of 15 descriptors on it.
        assert_equal(len(basic0.listdescriptors()["descriptors"]), 15)
        ...
        # FJAR: No bech32/SegWit - test with legacy (P2PKH) address instead
        addr_info = basic0.getaddressinfo(basic0.getnewaddress("", "legacy"))
        ...
        # FJAR: Only P2PKH addresses - no SegWit output types
        basic2_balance = 0
    ...
    def test_no_privkeys(self):
        ...
        # FJAR: Use legacy (P2PKH) addresses instead of bech32
        addr1 = default.getnewaddress(address_type="legacy")
        addr2 = default.getnewaddress(address_type="legacy")
        ...
        assert_equal(watchonly1.getnewaddress(address_type="legacy"), addr1)
    ...
    def run_test(self):
        ...
        # FJAR: test_conflict_txs requires RBF which FJAR does not support
        # self.test_conflict_txs()
        self.test_hybrid_pubkey()
        self.test_failed_migration_cleanup()
        self.test_avoidreuse()
        # FJAR: test_preserve_tx_extra_info requires bumpfee/RBF which FJAR does not support
