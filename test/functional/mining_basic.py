...
class MiningTest(BitcoinTestFramework):
    ...
    def test_blockmintxfee_parameter(self):
        ...
        for blockmintxfee_sat_kvb in (DEFAULT_BLOCK_MIN_TX_FEE, 0, 50, 100, 500, 2500, 5000, 21000, 333333, 2500000):
            blockmintxfee_bc2_kvb = blockmintxfee_sat_kvb / Decimal(COIN)
            ...
            tx_with_min_feerate = self.wallet.send_self_transfer(from_node=node, fee_rate=blockmintxfee_bc2_kvb)
            ...
            if blockmintxfee_bc2_kvb > 0:
                lowerfee_bc2_kvb = blockmintxfee_bc2_kvb - Decimal(10)/COIN  # 0.01 sat/vbyte lower
                tx_below_min_feerate = self.wallet.send_self_transfer(from_node=node, fee_rate=lowerfee_bc2_kvb)
                ...
                tx_below_min_feerate = self.wallet.send_self_transfer(from_node=node, fee_rate=blockmintxfee_bc2_kvb)
    ...
    def run_test(self):
        ...
        self.log.info("getblocktemplate: Verify no witness commitment (FJAR has no SegWit)")
        txid = int(self.wallet.send_self_transfer(from_node=node)['wtxid'], 16)
        tmpl = node.getblocktemplate(NORMAL_GBT_REQUEST_PARAMS)

        # FJAR does not have SegWit, so no witness commitment should be present
        ...
        self.log.info("getblocktemplate: FJAR does not require segwit rule")
