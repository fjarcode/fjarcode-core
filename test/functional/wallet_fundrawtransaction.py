...
class RawTransactionsTest(BitcoinTestFramework):
    ...
    def lock_outputs_type(self, wallet, outputtype):
        ...
        # FJAR: Lock UTXOs that do NOT match the given type, so only the
        # specified type remains available for coin selection.
        to_lock = []
        for utxo in wallet.listunspent():
            if "desc" in utxo:
                matches = any(utxo["desc"].startswith(prefix) for prefix in prefixes)
                if not matches:
    ...
    def test_duplicate_outputs(self):
        ...
        addr = w.getnewaddress(address_type="legacy")
    ...
    def test_invalid_change_address(self):
        ...
        assert_raises_rpc_error(-5, "Change address must be a valid FJAR address", self.nodes[2].fundrawtransaction, rawtx, changeAddress='foobar')
    ...
    def test_change_type(self):
        ...
        rawtx = self.nodes[2].fundrawtransaction(rawtx, change_type='legacy')
        dec_tx = self.nodes[2].decoderawtransaction(rawtx['hex'])
        assert_equal('pubkeyhash', dec_tx['vout'][rawtx['changepos']]['scriptPubKey']['type'])
    ...
    def test_locked_wallet(self):
        ...
        if self.options.descriptors:
            with WalletUnlock(wallet, "test"):
                # FJAR: Use pkh() descriptors instead of wpkh() (no SegWit)
        ...
        tx_size = 192  # FJAR: P2PKH -> P2PKH: 10 (overhead) + 148 (input) + 34 (output) = 192 bytes
    ...
    def test_option_feerate(self):
        self.log.info("Test fundrawtxn with explicit fee rates (fee_rate sat/vB and feeRate FJAR/kvB)")
        node = self.nodes[3]
        ...
        bc2_kvb_to_sat_vb = 100000  # (1e5)
        result1 = node.fundrawtransaction(rawtx, fee_rate=str(2 * bc2_kvb_to_sat_vb * self.min_relay_tx_fee))
        result2 = node.fundrawtransaction(rawtx, feeRate=2 * self.min_relay_tx_fee)
        result3 = node.fundrawtransaction(rawtx, fee_rate=10 * bc2_kvb_to_sat_vb * self.min_relay_tx_fee)
        ...
        # FJAR: P2PKH 1-in 2-out tx = 10 + 147 + 34*2 = 225 bytes. At ~1 sat/B, expect ~225 sat.
        assert_approx(node.fundrawtransaction(rawtx)["fee"], vexp=0.00000225, vspan=0.00000002)
        # Expect fee to be 10,000x higher when an explicit fee rate 10,000x greater is specified.
        result = node.fundrawtransaction(rawtx, fee_rate=10000)
        assert_approx(result["fee"], vexp=0.0225, vspan=0.001)
        ...
        assert_raises_rpc_error(-8, "Cannot specify both fee_rate (sat/vB) and feeRate (FJAR/kvB)",
    ...
    def test_option_subtract_fee_from_outputs(self):
        ...
        # Test subtract fee from outputs with feeRate (FJAR/kvB)
        ...
        result = [self.nodes[3].fundrawtransaction(rawtx),  # uses self.min_relay_tx_fee (set by settxfee)
            self.nodes[3].fundrawtransaction(rawtx, subtractFeeFromOutputs=[]),  # empty subtraction list
            self.nodes[3].fundrawtransaction(rawtx, subtractFeeFromOutputs=[0]),  # uses self.min_relay_tx_fee (set by settxfee)
            self.nodes[3].fundrawtransaction(rawtx, fee_rate=2 * bc2_kvb_to_sat_vb * self.min_relay_tx_fee),
            self.nodes[3].fundrawtransaction(rawtx, fee_rate=2 * bc2_kvb_to_sat_vb * self.min_relay_tx_fee, subtractFeeFromOutputs=[0]),]
        dec_tx = [self.nodes[3].decoderawtransaction(tx_['hex']) for tx_ in result]
        output = [d['vout'][1 - r['changepos']]['value'] for d, r in zip(dec_tx, result)]
        change = [d['vout'][r['changepos']]['value'] for d, r in zip(dec_tx, result)]
    ...
    def test_transaction_too_large(self):
        ...
        # Make 1500 0.1 FJAR outputs. The amount that we target for funding is in
    ...
    def test_external_inputs(self):
        ...
        # FJAR: WITNESS_SCALE_FACTOR=1 so min weight = 42 (41 bytes outpoint+sequence+empty scriptSig + 1 empty witness)
        assert_raises_rpc_error(-8, "Invalid parameter, weight cannot be less than 42", wallet.fundrawtransaction, raw_tx, input_weights=[{"txid": ext_utxo["txid"], "vout": ext_utxo["vout"], "weight": 41}])
        assert_raises_rpc_error(-8, "Invalid parameter, weight cannot be less than 42", wallet.fundrawtransaction, raw_tx, input_weights=[{"txid": ext_utxo["txid"], "vout": ext_utxo["vout"], "weight": -1}])
        ...
        # FJAR: WITNESS_SCALE_FACTOR=1, so weight = size; multiply by 1 instead of 4
        input_weight = signed_weight - unsigned_weight + (41 * 1)
        ...
        # FJAR: WITNESS_SCALE_FACTOR=1, so weight = size; multiply by 1 instead of 4
        input_add_weight = high_input_weight - (41 * 1)
        tx4_weight = wallet.decoderawtransaction(funded_tx4["hex"])["weight"] + input_add_weight
        # FJAR: weight = vsize (no witness discount)
        tx4_vsize = tx4_weight
    ...
    def test_add_inputs_default_value(self):
        ...
        # Fund wallet with 2 outputs, 5 FJAR each.
        addr2 = wallet.getnewaddress(address_type="legacy")
    ...
    def test_preset_inputs_selection(self):
        ...
        # Create and fund the wallet with 4 UTXO of 5 FJAR each (20 FJAR total)
        self.nodes[2].createwallet("test_preset_inputs_selection")
        wallet = self.nodes[2].get_wallet_rpc("test_preset_inputs_selection")
        outputs = {}
        for _ in range(4):
            outputs[wallet.getnewaddress(address_type="legacy")] = 5
        ...
        # Attempt to send 29 FJAR from a wallet that only has 20 FJAR. The wallet should exclude
        # the preset inputs from the pool of available coins, realize that there is not enough
        # money to fund the 29 FJAR payment, and fail with "Insufficient funds".
        #
        # Even with SFFO, the wallet can only afford to send 20 FJAR.
        # If the wallet does not properly exclude preset inputs from the pool of available coins
        # prior to coin selection, it may create a transaction that does not fund the full payment
        # amount or, through SFFO, incorrectly reduce the recipient's amount by the difference
        # between the original target and the wrongly counted inputs (in this case 9 FJAR)
        # so that the recipient's amount is no longer equal to the user's selected target of 29 FJAR.

        # First case, use 'subtract_fee_from_outputs = true'
        assert_raises_rpc_error(-4, "Insufficient funds", wallet.send, outputs=[{wallet.getnewaddress(address_type="legacy"): 29}], options=options)
    ...
    def test_weight_calculation(self):
        ...
        addr = wallet.getnewaddress(address_type="legacy")
        ext_addr = self.nodes[0].getnewaddress(address_type="legacy")
        utxo, ext_utxo = self.create_outpoints(self.nodes[0], outputs=[{addr: 5}, {ext_addr: 5}])

        self.nodes[0].sendtoaddress(wallet.getnewaddress(address_type="legacy"), 5)
        self.generate(self.nodes[0], 1)

        rawtx = wallet.createrawtransaction([utxo], [{self.nodes[0].getnewaddress(address_type="legacy"): 8}])
        fundedtx = wallet.fundrawtransaction(rawtx, fee_rate=10, change_type="legacy")
        # FJAR P2PKH: with 71-byte signatures (wallet can grind R)
        # tx overhead (10) + 2 P2PKH inputs (147 each: outpoint 36 + scriptSig_len 1 + scriptSig 106 + sequence 4) + 2 P2PKH outputs (34 each)
        tx_size = 10 + 147*2 + 34*2
        assert_equal(fundedtx['fee'] * COIN, tx_size * 10)

        # Using the other output should have 72 byte sigs (external input uses max sig)
        ...
        fundedtx = wallet.fundrawtransaction(rawtx, fee_rate=10, change_type="legacy", solving_data={"descriptors": [ext_desc]})
        # tx overhead (10) + 2 P2PKH wallet inputs (147 each, 71-byte sig) + 1 P2PKH external input (148, 72-byte sig) + 2 P2PKH outputs (34 each)
        tx_size = 10 + 147*2 + 148 + 34*2
        assert_equal(fundedtx['fee'] * COIN, tx_size * 10)
    ...
    def test_22670(self):
        ...
        # that this bug is not detected because the transaction fee must be at least 0.01 FJAR (the minimum change value).
        # Otherwise the targeted minimum change value will be enough to cover the transaction fees that were not
        # being accounted for. So the minimum relay fee is set to 0.1 FJAR/kvB in this test.
    ...
    def test_feerate_rounding(self):
        ...
        # FJAR: A P2PKH input costs 148 bytes; With a single P2PKH output (34 bytes) + overhead (10 bytes) = 192 bytes total.
        # At a feerate of 1.85 sat/B, the input will need a fee of 273.8 sats and the rest 81.4 sats
        # The entire tx fee should be 355.2 sats.
        # Coin selection rounds the fee individually instead of at the end (due to how CFeeRate::GetFee works).
        # If rounding down (which is the incorrect behavior), then the calculated fee will be 273 + 81 = 354.
        # If rounding up, then the calculated fee will be 274 + 82 = 356.
        # In the former case, the calculated needed fee is higher than the actual fee being paid, so an assertion is reached
        # To test this does not happen, we subtract 354 sats from the input value. If working correctly, this should
        # fail with insufficient funds rather than fjarcoded asserting.
        rawtx = w.createrawtransaction(inputs=[], outputs=[{self.nodes[0].getnewaddress(address_type="legacy"): 1 - 0.00000354}])
    ...
    def test_input_confs_control(self):
        ...
        # Create a replacement tx to 'final_tx1' that has 1 FJAR target instead of 0.1.
        ...
        # Now fund 'raw_tx2' to fulfill the total target (1 FJAR) by using all the wallet unconfirmed outputs.
        # As it was created with the first unconfirmed output, 'raw_tx2' only has 0.1 FJAR covered (need to fund 0.9 FJAR more).
