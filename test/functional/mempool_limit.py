...
class MempoolLimitTest(BitcoinTestFramework):
    ...
    def test_mid_package_eviction(self):
        ...
        # Specific number of satooshis to fit within a small window. The parent_cpfp + child package needs to be
        ...
        magic_satooshis = 1200
        cpfp_satooshis = int(cpfp_fee * COIN) + magic_satooshis

        child = self.wallet.create_self_transfer_multi(utxos_to_spend=parent_utxos, fee_per_output=cpfp_satooshis)
    ...
    def run_test(self):
        ...
        child_fee = (worst_feerate_bc2vb) * (target_weight_each // 4) - Decimal("0.00001")
        ...
        assert_greater_than(worst_feerate_bc2vb, (parent_fee + child_fee) / (tx_parent_just_below["tx"].get_vsize() + tx_child_just_above["tx"].get_vsize()))
