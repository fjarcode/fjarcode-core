...
class InvalidMessagesTest(BitcoinTestFramework):
    ...
    def test_size(self):
        # Skip with v2 transport since FJAR's 33MB limit makes allocating the
        # full payload impractical; the v1 raw-header approach is sufficient.
        if self.options.v2transport:
            return
        self.log.info("Test message with oversized payload disconnects peer")
        conn = self.nodes[0].add_p2p_connection(P2PDataStore())
        # FJAR: MAX_PROTOCOL_MESSAGE_LENGTH is 33MB (for 32MB blocks).
        # Instead of allocating a huge buffer, craft a raw P2P header with a
        # fake oversized size field. The node checks the header size before
        # reading the payload, so we only need to send the 24-byte header.
        fake_size = MAX_PROTOCOL_MESSAGE_LENGTH + 1
        error_msg = [f'Header error: Size too large (badmsg, {fake_size} bytes)']
        with self.nodes[0].assert_debug_log(error_msg):
            # Build a raw V1 header with the fake size
            magic = conn.magic_bytes
            command = b'badmsg\x00\x00\x00\x00\x00\x00'  # 12 bytes, padded
            size_bytes = struct.pack("<I", fake_size)
            checksum = b'\x00\x00\x00\x00'  # doesn't matter, size is checked first
            raw_header = magic + command + size_bytes + checksum
            conn.send_raw_message(raw_header)
    ...
    def test_addrv2(self, label, required_log_messages, raw_addrv2):
        ...
        # Make sure fjarcoded signals support for ADDRv2, otherwise this test
