...
class ScantxoutsetTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(self.nodes[0].scantxoutset("start", ["combo(" + pubk1.hex() + ")", "combo(" + pubk2.hex() + ")", "combo(" + pubk3.hex() + ")"])['total_amount'], Decimal("0.002"))
        assert_equal(self.nodes[0].scantxoutset("start", ["addr(" + addr_LEGACY + ")"])['total_amount'], Decimal("0.002"))
        ...
        # Note that all amounts in the UTXO set are powers of 2 multiplied by 0.001 FJAR, so each amounts uniquely identifies a subset.
