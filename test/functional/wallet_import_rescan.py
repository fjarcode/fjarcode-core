...
import RPC call and sends FJAR to it. Then other nodes import the addresses,
and the test makes listtransactions and getbalance calls to confirm that the
importing node either did or did not execute rescans picking up the send
transactions.

In the second part of the test, node 0 sends more FJAR to each address, and the
...
class Variant(collections.namedtuple("Variant", "call data address_type rescan prune")):
    ...
    def do_import(self, timestamp):
        ...
        # FJAR: Only legacy (P2PKH) addresses exist - no SegWit
        assert_equal(self.address["isscript"], False)
        assert_equal(self.address["iswitness"], False)
        ...
        if self.call == Call.single:
            ...
            # FJAR: No p2sh_segwit redeemscript handling needed (no SegWit)
...
# FJAR: Only legacy (P2PKH) address type - no SegWit (bech32, p2sh-segwit)
IMPORT_VARIANTS = [Variant(*variants) for variants in itertools.product(Call, Data, [AddressType.legacy], Rescan, (False, True))]
...
class ImportRescanTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        for i, variant in enumerate(mempool_variants):
            ...
            # 500 vbytes and feerate of 20 sat2s/vbytes
        ...
        for variant in mempool_variants:
            ...
            child = self.nodes[1].send(
                ...
                outputs=[{ADDRESS_FJARCODE_UNSPENDABLE : variant.initial_amount}],
