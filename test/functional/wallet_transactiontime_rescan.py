...
class TransactionTimeRescanTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        minernode = self.nodes[0]  # node used to mine FJAR and create transactions
        ...
        # generate some FJAR to create transactions and check blockcount
        ...
        # send 10 FJAR to user's first watch-only address
        self.log.info('Send 10 FJAR to user')
        ...
        # send 5 FJAR to our second watch-only address
        self.log.info('Send 5 FJAR to user')
        ...
        # send 1 FJAR to our third watch-only address
        self.log.info('Send 1 FJAR to user')
        ...
        if not self.options.descriptors:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as thread:
                with minernode.assert_debug_log(expected_msgs=["Rescan started from block 566b9aa69ef7720ada4eeac2149f3d2aefba80d4a1e41146981686e3b544012a... (slow variant inspecting all blocks)"], timeout=5):
