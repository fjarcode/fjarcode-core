...
class WalletSendTest(BitcoinTestFramework):
    ...
    def test_send(self, from_wallet, to_wallet=None, amount=None, data=None,
        ...
        if replaceable is None:
            replaceable = False # FJAR: no RBF support, always non-replaceable
        else:
        ...
        if add_to_wallet and not include_watching:
            ...
            tx = from_wallet.gettransaction(res["txid"])
            assert tx
            assert_equal(tx["bip125-replaceable"], "no")  # FJAR: no RBF, always "no"
    ...
    def run_test(self):
        ...
        for target, mode in product([-1, 0], ["fjar/kb", "sat/b"]):
        ...
        self.test_send(from_wallet=w0, to_wallet=w1, amount=1, change_address="not an address",
                       expect_error=(-5, "Change address must be a valid FJAR address"))
        ...
        # FJAR: CashAddr format, P2PKH addresses start with "fjarcode:q"
        assert change_address.startswith("fjarcode:q"), f"Expected CashAddr P2PKH, got: {change_address}"
        ...
        # FJAR: RBF is not supported, bip125-replaceable is always "no"
        self.log.info("Replaceable (FJAR: no RBF, always non-replaceable)...")
        ...
        # FJAR: "External outputs" section skipped - uses sh(wsh(pkh())) SegWit descriptors
        # which are not supported in FJAR (no SegWit/witness)
        self.log.info("External outputs (FJAR: skipped, requires SegWit descriptors)")
