...
class TestP2PConn(P2PInterface):
    ...
    def on_getdata(self, message):
        for i in message.inv:
            if i.type & MSG_TYPE_MASK == MSG_TX or i.type & MSG_TYPE_MASK == MSG_TX:
...
class TxDownloadTest(BitcoinTestFramework):
    ...
    def test_txid_inv_delay(self, glob_wtxid=False):
        ...
        if glob_wtxid:
            ...
            self.nodes[0].add_p2p_connection(TestP2PConn(wtxidrelay=False))
    ...
    def test_large_inv_batch(self):
        ...
        peer = self.nodes[0].add_p2p_connection(TestP2PConn())
        peer.send_message(msg_inv([CInv(t=MSG_TX, h=wtxid) for wtxid in range(MAX_PEER_TX_ANNOUNCEMENTS + 1)]))
    ...
    def run_test(self):
        ...
        # Run each test against new fjarcoded instances, as setting mocktimes has long-term effects on when
