...
dnl Output: If qt version is auto, set fjarcode_enable_qt to false. Else, exit.
AC_DEFUN([FJARCODE_QT_FAIL],[
  if test "$fjarcode_qt_want_version" = "auto" && test "$fjarcode_qt_force" != "yes"; then
    if test "$fjarcode_enable_qt" != "no"; then
      AC_MSG_WARN([$1; fjarcode-qt frontend will not be built])
    fi
    fjarcode_enable_qt=no
    fjarcode_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])

AC_DEFUN([FJARCODE_QT_CHECK],[
  if test "$fjarcode_enable_qt" != "no" && test "$fjarcode_qt_want_version" != "no"; then
    true
    $1
  else
    true
...
dnl FJARCODE_QT_PATH_PROGS([FOO], [foo foo2], [/path/to/search/first], [continue if missing])
...
AC_DEFUN([FJARCODE_QT_PATH_PROGS],[
  FJARCODE_QT_CHECK([
    if test "$3" != ""; then
      AC_PATH_PROGS([$1], [$2], [], [$3])
    else
    ...
    if test "$$1" = "" && test "$4" != "yes"; then
      FJARCODE_QT_FAIL([$1 not found])
    fi
  ])
])

dnl Initialize qt input.
dnl This must be called before any other FJARCODE_QT* macros to ensure that
dnl input variables are set correctly.
dnl CAUTION: Do not use this inside of a conditional.
AC_DEFUN([FJARCODE_QT_INIT],[
  dnl enable qt support
  AC_ARG_WITH([gui],
    [AS_HELP_STRING([--with-gui@<:@=no|qt5|auto@:>@],
    [build fjarcode-qt GUI (default=auto)])],
    [
     fjarcode_qt_want_version=$withval
     if test "$fjarcode_qt_want_version" = "yes"; then
       fjarcode_qt_force=yes
       fjarcode_qt_want_version=auto
     fi
    ],
    [fjarcode_qt_want_version=auto])
  ...
  AS_IF([test "$with_gui" = "qt5_debug"],
        [AS_CASE([$host],
                 ...
                 [qt_lib_suffix= ]); fjarcode_qt_want_version=qt5],
        [qt_lib_suffix= ])
...
dnl
dnl   FJARCODE_QT_CONFIGURE([MINIMUM-VERSION])
dnl
dnl Outputs: See _CODEFALCON_QT_FIND_LIBS
dnl Outputs: Sets variables for all qt-related tools.
dnl Outputs: fjarcode_enable_qt, fjarcode_enable_qt_dbus, fjarcode_enable_qt_test
AC_DEFUN([FJARCODE_QT_CONFIGURE],[
  qt_version=">= $1"
  qt_lib_prefix="Qt5"
  FJARCODE_QT_CHECK([_CODEFALCON_QT_FIND_LIBS])
  ...
  if test "$fjarcode_cv_static_qt" = "yes"; then
  ...
  FJARCODE_QT_PATH_PROGS([MOC], [moc-qt5 moc5 moc], $qt_bin_path)
  FJARCODE_QT_PATH_PROGS([UIC], [uic-qt5 uic5 uic], $qt_bin_path)
  FJARCODE_QT_PATH_PROGS([RCC], [rcc-qt5 rcc5 rcc], $qt_bin_path)
  FJARCODE_QT_PATH_PROGS([LRELEASE], [lrelease-qt5 lrelease5 lrelease], $qt_bin_path)
  FJARCODE_QT_PATH_PROGS([LUPDATE], [lupdate-qt5 lupdate5 lupdate],$qt_bin_path, yes)
  FJARCODE_QT_PATH_PROGS([LCONVERT], [lconvert-qt5 lconvert5 lconvert], $qt_bin_path, yes)
  ...
  FJARCODE_QT_CHECK([
    fjarcode_enable_qt=yes
    fjarcode_enable_qt_test=yes
    if test "$have_qt_test" = "no"; then
      fjarcode_enable_qt_test=no
    fi
    fjarcode_enable_qt_dbus=no
    if test "$use_dbus" != "no" && test "$have_qt_dbus" = "yes"; then
      fjarcode_enable_qt_dbus=yes
    fi
  ...
  if test $fjarcode_enable_qt = "yes"; then
    AC_MSG_RESULT([$fjarcode_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$fjarcode_enable_qt])
  fi
...
dnl Output: fjarcode_cv_static_qt=yes|no
AC_DEFUN([_CODEFALCON_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, fjarcode_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
        #ifndef QT_VERSION
        #  include <QtCore/qglobal.h>
      ...
      [fjarcode_cv_static_qt=yes],
      [fjarcode_cv_static_qt=no])
    ])
])
...
AC_DEFUN([_CODEFALCON_QT_CHECK_STATIC_PLUGIN], [
  ...
  AC_LINK_IFELSE([AC_LANG_PROGRAM([[
    ...
    [AC_MSG_RESULT([no]); FJARCODE_QT_FAIL([$1 not found.])])
...
AC_DEFUN([_CODEFALCON_QT_FIND_LIBS],[
  FJARCODE_QT_CHECK([
    PKG_CHECK_MODULES([QT_CORE], [${qt_lib_prefix}Core${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_CORE_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_CORE_LIBS $QT_LIBS"],
                      [FJARCODE_QT_FAIL([${qt_lib_prefix}Core${qt_lib_suffix} $qt_version not found])])
  ])
  FJARCODE_QT_CHECK([
    PKG_CHECK_MODULES([QT_GUI], [${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_GUI_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_GUI_LIBS $QT_LIBS"],
                      [FJARCODE_QT_FAIL([${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version not found])])
  ])
  FJARCODE_QT_CHECK([
    PKG_CHECK_MODULES([QT_WIDGETS], [${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_WIDGETS_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_WIDGETS_LIBS $QT_LIBS"],
                      [FJARCODE_QT_FAIL([${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version not found])])
  ])
  FJARCODE_QT_CHECK([
    PKG_CHECK_MODULES([QT_NETWORK], [${qt_lib_prefix}Network${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_NETWORK_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_NETWORK_LIBS $QT_LIBS"],
                      [FJARCODE_QT_FAIL([${qt_lib_prefix}Network${qt_lib_suffix} $qt_version not found])])
  ])
